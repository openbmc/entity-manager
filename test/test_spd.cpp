...
TEST(ValidateSPDContentTest, SizeNotEnough)
{
    ...
    EXPECT_EQ(SPD::getDRAMType(spdContent), DRAMType::dramTypeUnknown);
...
TEST(ValidateSPDContentTest, TypeUnsupported)
{
    ...
    const uint8_t type = static_cast<uint8_t>(DRAMType::dramTypeDDR1);
...
{
    const uint8_t type = static_cast<uint8_t>(DRAMType::dramTypeUnknown);
    const std::vector<uint8_t> spdContent = {0x30, 0x09, type};
    EXPECT_EQ(SPD::getFromImage(spdContent), nullptr);
}
