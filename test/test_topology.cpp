...
const std::string subchassisPath = "/xyz/openbmc_project/inventory/system/chassis/Subchassis";
const std::string superchassisPath = "/xyz/openbmc_project/inventory/system/chassis/Superchassis";

const Association subchassisAssoc = std::make_tuple("chassisContainedBy", "chassisContains",
                   superchassisPath);
...
{
    const Association subchassisAssoc2 = std::make_tuple("chassisContainedBy", "chassisContains",
                       superchassisPath + "2");
    ...
    EXPECT_THAT(assocs[subchassisPath], UnorderedElementsAre(subchassisAssoc, subchassisAssoc2));
}
...
TEST(Topology, 2SuperchassisAnd2Subchassis)
{
    ...
    EXPECT_THAT(assocs[subchassisPath], UnorderedElementsAre(subchassisAssoc, subchassisAssoc2));
    EXPECT_THAT(assocs[subchassisPath + "2"], UnorderedElementsAre(subchassisAssoc, subchassisAssoc2));
}
