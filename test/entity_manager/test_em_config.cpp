...
TEST(EMConfig, MissingName)
{
    nlohmann::json::object_t input = getSampleConfig();
    ASSERT_EQ(input.erase("Name"), 1);

    std::optional<EMConfig> optConfig = EMConfig::fromJson(input);
    ASSERT_FALSE(optConfig.has_value());
}

TEST(EMConfig, MissingType)
{
    nlohmann::json::object_t input = getSampleConfig();
    ASSERT_EQ(input.erase("Type"), 1);
