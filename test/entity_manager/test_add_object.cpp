...
TEST_F(AddObjectTest, AddObjectPersistsDynamicDoesNot)
{
    ...
    em->dbus_interface.addObjectJson(data, em->systemConfiguration,
                                     kBoardJsonPtr,
                                     sdbusplus::object_path{kBoardDbusPath},
                                     kBoardId, /*isDynamic=*/false);
    io.poll();
    ...
    em->dbus_interface.addObjectJson(dynData, em->systemConfiguration,
                                     kBoardJsonPtr,
                                     sdbusplus::object_path{kBoardDbusPath},
                                     kBoardId, /*isDynamic=*/true);
    io.poll();

    {
        std::ifstream f{tmpDir / "system.json"};
        ASSERT_TRUE(f.is_open())
            << "system.json must survive AddObjectDynamic";
        auto onDisk = nlohmann::json::parse(f);
