...
TEST(FormatIPMIFRUTest, AreaOffsetPastBufferReturnsErr)
{
    // 16-byte FRU where the chassis offset byte is 0xFF, resolving to an
    // offset of 0xFF * 8 = 2040 bytes - far past the buffer. The new
    // offset guard must reject it; the parse must not proceed to the
    // iterator checks (which happen to also reject this, but only as a
    // side effect).
    std::vector<uint8_t> fru(16, 0x00);
    fru[0] = 0x01; // common header format version
    fru[2] = 0xFF; // chassis area offset: 0xFF * 8 = 2040 bytes

    std::flat_map<std::string, std::string, std::less<>> result;
    EXPECT_EQ(formatIPMIFRU(fru, result), resCodes::resErr);
}

TEST(FormatIPMIFRUTest, AreaOffsetAtLastBlockNotFalselyRejected)
{
    // Reverse control for the offset guard, reusing the exact 17-byte
    // layout of ValidAreaNearBufferEndAccepted: a valid FRU whose area
    // occupies the last full block must stay accepted. Both the guard
    // and the pre-existing iterator checks accept this input today; the
    // test pins the boundary so a future over-strict guard (e.g. one
    // requiring a spare byte, offset + fruBlockSize + 1 >= size) fails
    // here instead of silently rejecting valid FRUs.
    const auto fru = std::to_array<uint8_t>({
        0x01,
        0x00,
        0x00,
        ...
        0x00,
        0x00,
        0xfe, // Header (board at 8, last full block in this 17-byte FRU)
        0x01,
        0x01,
        0x19,
        0x00,
        ...
        0xc1,
        0x24, // board area, 1 block
        0x00, // trailing padding byte past the last full block
    });
