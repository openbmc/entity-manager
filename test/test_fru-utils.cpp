...
int64_t getDataTempl(
    ...
    off_t offset, size_t length, uint8_t* outBuf)
{
...
TEST(FRUReaderTest, ReadData)
{
    ...
    EXPECT_EQ(reader.read(0, data.size(), rdbuf.data()), static_cast<ssize_t>(data.size()));
...
TEST(FRUReaderTest, StartPastKnownEOF)
{
    ...
    EXPECT_EQ(reader.read(0, blockSize, blockData.data()), static_cast<ssize_t>(data.size()));
...
TEST(FRUReaderTest, ReadPastKnownEnd)
{
    ...
    EXPECT_EQ(rdbuf[0], 'X');
    EXPECT_EQ(rdbuf[1], 'Y');
    EXPECT_EQ(reader.read(1, data.size(), rdbuf.data()), static_cast<ssize_t>(data.size() - 1));
    EXPECT_EQ(rdbuf[0], 'Y');
}
...
TEST(FRUReaderTest, MultiBlockRead)
{
    ...
    EXPECT_EQ(reader.read(0, 2 * blockSize, rdbuf.data()), static_cast<ssize_t>(2 * blockSize));
    EXPECT_TRUE(std::equal(rdbuf.begin(), rdbuf.end(), data.begin()));
}
