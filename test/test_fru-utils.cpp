...
TEST(FRUReaderTest, StartPastUnknownEOF)
{
    ...
    EXPECT_EQ(reader.read(1, 1, NULL), 0);
}
...
TEST(FRUReaderTest, StartPastKnownEOF)
{
    ...
    EXPECT_EQ(reader.read(data.size(), 1, NULL), 0);
    EXPECT_EQ(reader.read(data.size() + 1, 1, NULL), 0);
    EXPECT_EQ(reader.read(blksz, 1, NULL), 0);
    EXPECT_EQ(reader.read(blksz + 1, 1, NULL), 0);
}
...
TEST(FRUReaderTest, DecreasingEOF)
{
    ...
    EXPECT_EQ(reader.read(blksz * 2, 1, NULL), 0);
    EXPECT_EQ(reader.read(blksz + (blksz / 2), 1, NULL), 0);
    EXPECT_EQ(reader.read(blksz, 1, NULL), 0);
    EXPECT_EQ(reader.read(blksz / 2, 1, NULL), 0);
    EXPECT_EQ(reader.read(0, 1, NULL), 0);
}
