...
scan::DBusDeviceDescriptor makeDevice(
    const std::string& path, const DBusInterface& interface = {})
{
...
{
    scan::FoundDevices set1;
    scan::FoundDevices set2 = {makeDevice("/path/1", iface2), makeDevice("/path/2", iface2)};
    probe::intersectDevices(set1, set2);
    EXPECT_TRUE(set1.empty());
}
...
{
    scan::FoundDevices set1 = {makeDevice("/path/1", iface1), makeDevice("/path/2", iface2)};
    scan::FoundDevices set2 = {makeDevice("/path/3", iface3), makeDevice("/path/4", iface4)};
    probe::intersectDevices(set1, set2);
    EXPECT_TRUE(set1.empty());
}
