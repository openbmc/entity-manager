...
TEST(TemplateCharReplace, replaceOneInt)
{
    ...
    auto it = j.begin();
    boost::container::flat_map<std::string, DBusValue> data;
    data["BUS"] = 23;
...
{
    nlohmann::json j = R"("-123.4")"_json;
    DBusValue v = std::vector<uint8_t>{1, 2};
    EXPECT_FALSE(matchProbe(j, v));
}
...
{
    nlohmann::json j = R"(11)"_json;
    DBusValue v = std::vector<uint8_t>{11};
    EXPECT_FALSE(matchProbe(j, v));
}
...
{
    nlohmann::json j = R"([1, 2])"_json;
    DBusValue v = std::vector<uint8_t>{1, 2, 3};
    EXPECT_FALSE(matchProbe(j, v));
}
...
TEST(MatchProbe, emptyArrayEqEmptyArray)
{
    ...
    DBusValue v = std::vector<uint8_t>{};
    EXPECT_TRUE(matchProbe(j, v));
}
...
{
    nlohmann::json j = R"([])"_json;
    DBusValue v = std::vector<uint8_t>{1};
    EXPECT_FALSE(matchProbe(j, v));
}
