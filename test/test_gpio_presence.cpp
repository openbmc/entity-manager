    ...
    -> sdbusplus::async::task<>
{
    ...
    // First add: populates gpioLines entry (even though find_line fails,
    // operator[] creates the map entry before the exception is thrown).
    // Do NOT call updatePresence -- c1 must not create a D-Bus interface
    // so there is no path conflict when c2 creates one.
    ...
    // Simulate readGPIOAsyncEvent having cached the GPIO state.
    sensor.gpioState[gpioName] = false;

    // Re-add: simulate config provider restart. The fix should create
