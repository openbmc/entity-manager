...
involve multiple USB hubs across entities. The **MCTPUSBDevice** config type
lives in `schemas/platform_schemas/mctp.json` (not in entity
`schemas/mctp.json`).
...
- **platform_exposes_record.json** — Defines the allowed types for each element
  of the PlatformExposes array (e.g. MCTPUSBDevice).
- **mctp.json** — Defines MCTPUSBDevice and other platform-only MCTP types.
