    ...
    std::shared_ptr<sdbusplus::asio::connection> systemBus)
{
    ...
    if (updatePropertyReqLen == minUpdatePropReqLen ||
        updatePropertyReqLen > maxUpdatePropReqLen)
    {
        std::cerr
    ...
    rescanBuses(busMap, dbusInterfaceMap, objServer, std::move(systemBus));
    ...
    const std::shared_ptr<sdbusplus::asio::connection>& systemBus)
{
    ...
    if (bus > 0)
    {
        ...
        if (found)
        {
            ...
            productName += "_" + std::to_string(++highest);
        }
    }
