...
ssize_t FRUReader::CachingReader::read(off_t start, size_t len, uint8_t* outbuf)
{
    size_t done = 0;
    ...
    while (done < len)
    {
        ...
        size_t toCopy = (blkOffset >= available) ? 0 : std::min(available - blkOffset, remaining);
                                                ...
                                                const std::string& errorHelp)
{
         ...
         area <= fruAreas::fruAreaMultirecord; ++area)
    {
        ...
        if (reader.read(areaOffset, areaData.size(), areaData.data()) != static_cast<ssize_t>(areaData.size()))
        {
            std::cerr << "failed to read " << errorHelp << "\n";
            return {};
        }
