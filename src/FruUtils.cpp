...
int64_t FRUReader::read(uint16_t start, uint8_t len, uint8_t* outbuf)
{
    if (noCache)
    {
        return readFunc(start, len, outbuf);
    }
                                                ...
                                                const std::string& errorHelp)
{
    ...
    if (reader.read(0, device.size(), device.data())
        != static_cast<int64_t>(device.size()))
    {
        return {};
    }
         ...
         area <= fruAreas::fruAreaMultirecord; ++area)
    {
        ...
        if (reader.read(static_cast<uint16_t>(areaOffset), areaData.size(),
                                                ...
                                                const std::string& errorHelp)
{
    ...
    if (reader.read(0, magicBytes.size(), magicBytes.data()) != magicBytes.size()
        || magicBytes != tyanMagic)
    {
        return {};
    }

    auto tyanReadFunc = [&reader](int64_t offset, uint8_t length, uint8_t* outbuf) {
        return reader.read(offset + 0x6000, length, outbuf);
    };
    FRUReader tyanReader(tyanReadFunc, true);
...
std::vector<uint8_t> readFRUContents(FRUReader& reader,
                                     const std::string& errorHelp)
{
    static constexpr auto fruParsers = std::to_array({
        readIPMIFRUContents,
        readTyanFRUContents,
    });
