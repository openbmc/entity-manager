...
static std::vector<uint8_t> li1902FRUBytes(const LI1902FRUData& fru)
{
    ...
    uint8_t chassis_offset = 1;
    uint8_t board_offset = chassis_offset + (chassis.size() / fruBlockSize);
    uint8_t product_offset = board_offset + (board.size() / fruBlockSize);
...
static std::vector<uint8_t> readLI19FRUContents(int flag, int file, uint16_t address,
                                                ...
                                                const std::string& errorHelp)
{
    ...
    {
        size_t to_read = std::min(data.size() - bytes_read,
                                  (size_t)I2C_SMBUS_BLOCK_MAX);
        if (readBlock(flag, file, address, bytes_read, to_read,
