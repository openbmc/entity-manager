...
bool updateFRUProperty(
    ...
    sdbusplus::asio::object_server& objServer, bool ipmbFlag,
    std::shared_ptr<sdbusplus::asio::connection> systemBus)
{
    ...
    else
    {
        ...
        // Rescan the bus so that GetRawFru dbus-call fetches updated values
        rescanBusses(busMap, dbusInterfaceMap, objServer, systemBus);
    }
    return true;
}
    ...
    bool ipmbFlag, std::shared_ptr<sdbusplus::asio::connection> systemBus)
{
    ...
    if (ipmbFlag)
    {
        if (bus == 0)
        {
            productName = ipmbFruDbusObject + productName + "_0";
        }
        else
        {
            productName = ipmbFruDbusObject + productName;
        }
    }
    else
    {
        productName = fruDbusObject + productName;
    }
    ...
    if (ipmbFlag)
    {
        iface = objServer.add_interface(productName, ipmbFruDbusInterface);
    }
    else
    {
        iface = objServer.add_interface(productName, fruDbusInterface);
    }
    ...
    for (auto& property : formattedFRU)
    {
        ...
        if (property.first == "PRODUCT_ASSET_TAG")
        {
            ...
            iface->register_property(
                ...
                [bus, address, propertyName, &dbusInterfaceMap, &objServer,
                 ipmbFlag,
