...
std::vector<uint8_t> getMimePayload(sdbusplus::bus_t& bus, const std::string& objPath)
{
    auto m = bus.new_method_call(
        NEARD_SERVICE, objPath.c_str(),
...
std::optional<std::string> findFruRecordPath(sdbusplus::bus_t& bus)
{
    ...
    while ((pos = xml.find("node name=\"tag", pos)) != std::string::npos)
    {
        ...
        auto m2 = bus.new_method_call(
            NEARD_SERVICE, tagPath.c_str(),
            INTROSPECT_INTERFACE, "Introspect");
