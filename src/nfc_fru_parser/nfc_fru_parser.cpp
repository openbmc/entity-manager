...
struct Context
{
    ...
    std::unique_ptr<sdbusplus::bus::match_t> recordMatch;
...
{
    getMimePayloadAsync(
        ctx.conn, ctx.recordPath,
        [&ctx](const std::vector<uint8_t>& payload) {
            if (payload.empty())
            {
                lg2::warning("Empty MIME payload or failed to get payload");
                return;
            }
...
static void readTagRecord(Context& ctx)
{
    ctx.recordMatch = startRecordMatch(
    ctx.conn,
