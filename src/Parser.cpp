...
using StringPieceType =
    std::vector<boost::spirit::x3::variant<std::string, NumericOp>>;
struct StringPiece
{
...
BOOST_FUSION_ADAPT_STRUCT(client::ast::NumericVariable, expr)
...
x3::symbols<int> symVal;
x3::symbols<std::string> symStr;

// Non function strings go straight through
x3::rule<class stringPiece, ast::StringPieceType> const stringPiece =
    "stringPiece";
...
BOOST_SPIRIT_DEFINE(stringPiece, expression, term, factor, stringOp, variableOp)
...
std::optional<std::string> parseAndEvaluate(
    std::string& str,
    std::vector<std::pair<std::string, std::string>>& replaceStr,
    std::vector<std::pair<std::string, int64_t>>& replaceVal)
{
    ...
    for (auto& iter : replaceStr)
    {
        client::entity_template_grammar::symStr.add(iter.first, iter.second);
    }

    for (auto& iter : replaceVal)
    {
        client::entity_template_grammar::symVal.add(iter.first, iter.second);
    }
    ...
    client::entity_template_grammar::symStr.clear();
    client::entity_template_grammar::symVal.clear();
