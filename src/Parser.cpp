...
x3::rule<class variableOp1, std::string> const variableOp1 = "variableOp1";
    ...
    std::vector<std::pair<std::string, int64_t>>& replaceVal)
{
    ...
    for (unsigned int i = 0; i < replaceStr.size(); i++)
    {
        client::entity_template_grammar::symStr.add(replaceStr[i].first,
                                                    replaceStr[i].second);
    }

    for (unsigned int i = 0; i < replaceVal.size(); i++)
    {
        client::entity_template_grammar::symVal.add(replaceVal[i].first,
                                                    replaceVal[i].second);
    }
