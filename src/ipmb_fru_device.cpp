                  ...
                  std::shared_ptr<sdbusplus::asio::connection> systemBus)
{
    ...
    ipmbWrite(ipmbAddress[bus], offset, fru, std::move(systemBus));
                 ...
                 std::shared_ptr<sdbusplus::asio::connection>& conn)
{
    ...
    for (const auto& pathPair : resp)
    {
        for (const auto& entry : pathPair.second)
        {
            ...
            devIndex = std::stoi(index);

            if (typeConfig == "me")
            {
                type = 1;
            }
            else if (typeConfig == "ipmb")
            {
                type = 0;
            }
            else
    ...
    sdbusplus::asio::object_server& objServer)
{
    ...
    for (auto& devicemap : busMap)
    {
        ...
        {
            if(addFruObjectToDbus(device.second, dbusInterfaceMap, devicemap.first,
                               device.first, unknownBusObjectCount, true, objServer, systemBus, devicemap.first, true))
            {
               std::cerr << "addFruObjectToDbus Failed for bus : " << devicemap.first <<std::endl;
               continue;
            }
        }
    }
