...
std::unique_ptr<SPD> SPD::parseSPD(std::span<const uint8_t> spdImage)
{
    ...
    if (type == Dimm::DeviceType::DDR5)
    {
        ...
        uint16_t storedCrc = spdImage[offsetof(DDR5Fields, crc)] |
                             spdImage[offsetof(DDR5Fields, crc) + 1];
...
std::string SPD::serialNumber() const
{
    ...
    for (size_t i = 0; i < sizeof(spd_.ddr5.serialNumber); i++)
    {
        result << static_cast<uint16_t>(spd_.ddr5.serialNumber[i]);
    }
    return result.str();
}
...
{
    std::string result;
    for (size_t i = 0; i < sizeof(spd_.ddr5.partNumber); i++)
    {
        if (std::isprint(spd_.ddr5.partNumber[i]) &&
            !std::isspace(spd_.ddr5.partNumber[i]))
        {
            result.push_back(spd_.ddr5.partNumber[i]);
        }
    }
    return result;
}
