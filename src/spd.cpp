...
std::unique_ptr<SPD> SPD::parseSPD(std::span<const uint8_t> spdImage)
{
            ...
            deviceType(spdImage[offsetof(SPDKeyFields, deviceType)])))
    {
        if (debug)
        {
            std::cerr << "parseSPD failed: "
                      << "spdImage.size() = " << spdImage.size()
                      << ", spdSize() = "
                      << spdSize(spdImage[offsetof(SPDKeyFields, spdSize)])
                             .value_or(0);
        }
        return nullptr;
    }
    ...
    if (type == Dimm::DeviceType::DDR5)
    {
                ...
                sizeof(decltype(std::declval<DDR5Fields>().crc)))
        {
            if (debug)
            {
                std::cerr << "SPD doesn't contain crc byte" << std::endl;
            }
            return nullptr;
        }
...
std::optional<std::string> SPD::manufacturerId() const
{
    if (!manufacturerID.contains(spd_.ddr5.dramManufacturerId))
    {
        if (debug)
        {
            std::cerr << "`dramManufacturerId` is invalid" << std::endl;
        }
        return std::nullopt;
    }
...
{
    constexpr uint8_t kMaxWeek = 53;

    auto yy = bcdToUint8(spd_.ddr5.moduleManufacturingDate[0]);
    auto ww = bcdToUint8(spd_.ddr5.moduleManufacturingDate[1]);

    if (yy == std::nullopt || ww == std::nullopt || ww > kMaxWeek)
    {
        if (debug)
        {
            std::cerr << "`moduleManufacturingDate` is invalid" << std::endl;
        }
        return std::nullopt;
    }
