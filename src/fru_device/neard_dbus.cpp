    ...
    const std::map<std::string, DBusValueVariant>& props)
{
    ...
    const auto* mimePayloadString = std::get_if<std::string>(&payloadIt->second);
    ...
    const std::map<std::string, DBusValueVariant>& props)
{
    ...
    if (uidIt == props.end())
    {
        lg2::error("Uid property missing for tag path {PATH}", "PATH",
                   static_cast<const std::string&>(tagPath));
        onUid(std::nullopt);
        return;
    }
    ...
    if (uid == nullptr)
    {
        lg2::error("Uid property has unexpected type for tag path {PATH}",
                   "PATH", static_cast<const std::string&>(tagPath));
        onUid(std::nullopt);
        return;
    }

    std::string uidString = uidToString(*uid);
    lg2::debug("Detected NFC tag UID {UID} at {PATH}", "UID", uidString,
              "PATH", static_cast<const std::string&>(tagPath));
    onUid(uidString);
}
...
static void getManagedObjectsCallback(
    ...
    const boost::system::error_code& ec,
    const DBusManagedObjectsType& objects)
{
    if (ec)
    {
    ...
    std::function<void(const std::vector<uint8_t>&)> payload)
{
    ...
    conn->async_method_call(
        std::move(callback),
        neardService, objPath, "org.freedesktop.DBus.Properties", "GetAll",
    ...
    std::function<void(const std::optional<std::string>&)> onUid)
{
    std::function<void(const boost::system::error_code&,
                       const std::map<std::string, DBusValueVariant>&)>
        callback = std::bind_front(getTagUidCallback, tagPath,
                                   std::move(onUid));

    conn->async_method_call(
        std::move(callback),
        neardService, tagPath, "org.freedesktop.DBus.Properties", "GetAll",
        neardTagInterface);
}
