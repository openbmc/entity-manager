    ...
    std::enable_shared_from_this<FindDevicesWithCallback>
{
    ...
    void run()
    {
        fruDevice->findI2CDevices(_i2cBuses, fruDevice->utils.busMap,
                                  _powerIsOn, _addressBlocklist, _objServer);
    }
    ...
    auto scan = std::make_shared<FindDevicesWithCallback>(
        ...
        [busNum, &dbusInterfaceMap, &unknownBusObjectCount, &powerIsOn,
         &objServer, &addressBlocklist, capturedFruDevice]() {
            ...
            auto found = capturedFruDevice->utils.busMap.find(busNum);
            if (found == capturedFruDevice->utils.busMap.end() ||
