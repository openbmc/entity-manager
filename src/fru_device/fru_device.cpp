...
void FruDevice::loadBlocklist(const char* path)
{
    ...
    std::enable_shared_from_this<FindDevicesWithCallback>
{
    FindDevicesWithCallback(
        ...
        FruDevice& fruDeviceIn, std::function<void()>&& callback) :
        _i2cBuses(i2cBuses), _busMap(busmap), _powerIsOn(powerIsOn),
        _objServer(objServer), fruDevice(fruDeviceIn),
    ...
    void run()
    {
        fruDevice.findI2CDevices(_i2cBuses, _busMap, _powerIsOn, _objServer);
    }
    ...
    if (ENABLE_FRU_UPDATE_PROPERTY)
    {
        iface->register_method(
            ...
            [bus, address, &dbusInterfaceMap, &unknownBusObjectCount,
             &powerIsOn, &objServer, this](const std::string& fieldName,
    ...
    sdbusplus::asio::object_server& objServer)
{
    ...
    auto scan = std::make_shared<FindDevicesWithCallback>(
        i2cBuses, utils.busMap, powerIsOn, objServer, *this,
        [busNum, this, &dbusInterfaceMap, &unknownBusObjectCount, &powerIsOn,
         &objServer]() {
            ...
            auto found = utils.busMap.find(busNum);
            if (found == utils.busMap.end() || found->second == nullptr)
            {
                return;
            }
        ...
        auto scan = std::make_shared<FindDevicesWithCallback>(
            i2cBuses, utils.busMap, powerIsOn, objServer, *this, [&]() {
                ...
                if (readBaseboardFRU(baseboardFRU))
                {
                    ...
                    auto bus0 = utils.busMap.try_emplace(
                        0, std::make_shared<DeviceMap>());
                    bus0.first->second->emplace(0, baseboardFRU);
                }
                for (auto& devicemap : utils.busMap)
                {
    ...
    sdbusplus::asio::object_server& objServer)
{
    ...
    rescanBusses(dbusInterfaceMap, unknownBusObjectCount, powerIsOn, objServer);
    return true;
}
...
int main()
{
    ...
    auto systemBus =
        std::make_shared<sdbusplus::asio::connection>(fruDevice.io);
    ...
    const std::function<std::vector<uint8_t>&(const uint16_t& bus,
                                              const uint8_t& address)>
        getRawFruFn = std::bind_front(&FruUtils::getFRUInfo, fruDevice.utils);
    ...
    boost::asio::posix::stream_descriptor dirWatch(fruDevice.io, fd);
