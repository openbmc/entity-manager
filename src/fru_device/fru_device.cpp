...
static void handleNfcPayload(
    ...
    NfcFruState& nfcState, const sdbusplus::object_path& tagPath,
    const NfcUid& uid, sdbusplus::asio::object_server& objServer)
{
    ...
    auto existing = nfcState.uidToFruKey.find(uid);
    if (existing != nfcState.uidToFruKey.end())
    {
    ...
    for (auto tagIt = nfcState.tagPathToUid.begin();
         tagIt != nfcState.tagPathToUid.end();)
    {
        if (tagIt->second == uid && tagIt->first != tagPath)
        {
            tagIt = nfcState.tagPathToUid.erase(tagIt);
        }
        else
        {
            ++tagIt;
    ...
    const std::pair<uint32_t, uint32_t> nfcKey{nfcBus, nfcState.nextAddress++};
    ...
    nfcState.tagPathToUid.insert_or_assign(tagPath, uid);
    nfcState.uidToFruKey.insert_or_assign(uid, nfcKey);
...
static void handleNfcTagAdded(
    ...
    FruDetails& fruDetails, NfcFruState& nfcState,
    sdbusplus::asio::object_server& objServer, sdbusplus::message_t& message)
{
    ...
    getNfcTagUidAsync(
        systemBus, *tagPath,
        [systemBus, path, tagPath = *tagPath, &fruDetails, &nfcState,
         &objServer](const std::optional<std::string>& uid) {
            ...
            getMimePayloadAsync(
                systemBus, path,
                [tagPath, nfcUid = std::move(nfcUid), &fruDetails, &nfcState,
                 &objServer](std::vector<uint8_t> payload) {
                    handleNfcPayload(payload, fruDetails, nfcState, tagPath,
                                     nfcUid, objServer);
                });
        });
}

static void handleNfcTagRemoved(FruDetails& fruDetails, NfcFruState& nfcState,
                                sdbusplus::asio::object_server& objServer,
                                sdbusplus::message_t& message)
{
    ...
    auto uidIt = nfcState.tagPathToUid.find(path);
    if (uidIt == nfcState.tagPathToUid.end())
    {
        return;
    }

    auto fruKeyIt = nfcState.uidToFruKey.find(uidIt->second);
    if (fruKeyIt == nfcState.uidToFruKey.end())
    {
        nfcState.tagPathToUid.erase(uidIt);
        return;
    }
    ...
    nfcState.uidToFruKey.erase(fruKeyIt);
    nfcState.tagPathToUid.erase(uidIt);
}
    ...
    sdbusplus::asio::object_server& objServer)
{
    queryNeardObjectsAsync(
        systemBus, [systemBus, &fruDetails, &nfcState,
                 ...
                 &objServer](const std::optional<std::string>& uid) {
                    ...
                    getMimePayloadAsync(
                        ...
                        [tagPath, nfcUid = std::move(nfcUid), &fruDetails,
                         &nfcState, &objServer](std::vector<uint8_t> payload) {
                            handleNfcPayload(payload, fruDetails, nfcState,
                                             tagPath, nfcUid, objServer);
                        });
                });
        });
}

static void setupNfcMonitor(
    const std::shared_ptr<sdbusplus::asio::connection>& systemBus,
    FruDetails& fruDetails, NfcFruState& nfcState,
    sdbusplus::asio::object_server& objServer,
    ...
    std::optional<sdbusplus::bus::match_t>& tagRemovedMatch)
{
    ...
    auto nfcTagAddedHandler = [systemBus, &fruDetails, &nfcState,
                               &objServer](sdbusplus::message_t& message) {
        handleNfcTagAdded(systemBus, fruDetails, nfcState, objServer, message);
    };

    auto nfcTagRemovedHandler =
        [&fruDetails, &nfcState, &objServer](sdbusplus::message_t& message) {
            handleNfcTagRemoved(fruDetails, nfcState, objServer, message);
        };
    ...
    queryNeardObjects(systemBus, fruDetails, nfcState, objServer);
}
...
int main()
{
    ...
    iface->register_method("ReScanBus", [&](uint16_t bus) {
        rescanOneBus(busMap, bus, fruDetails, true, objServer);
    });
    ...
    setupNfcMonitor(systemBus, fruDetails, nfcState, objServer,
                    nfcTagAddedMatch, nfcTagRemovedMatch);
