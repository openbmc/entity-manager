                             ...
                             sdbusplus::asio::object_server& objServer)
{
    ...
    const auto nfcKey = std::make_pair(
        static_cast<size_t>(nfcBus), fruDetails.nextNfcAddress++);
    ...
    sdbusplus::message_t& message)
{
    ...
    std::map<
        std::string,
        std::map<std::string, DBusValueVariant>>
        interfaces;
            ...
            getMimePayloadAsync(
                ...
                [tagPath, uid = *uid, &fruDetails,
                 &objServer](std::vector<uint8_t> payload) {
                    handleNfcPayload(payload, fruDetails, tagPath, uid,
                                     objServer);
                });
        });
}
