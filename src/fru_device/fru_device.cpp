...
static void handleNfcPayload(
    ...
    const sdbusplus::object_path& tagPath, const NfcUid& uid,
    sdbusplus::asio::object_server& objServer)
{
    if (payload.empty())
    {
    ...
    getNfcTagUidAsync(systemBus, *tagPath,
                      [systemBus, path, tagPath = *tagPath, &fruDetails,
                       ...
                       &objServer](const std::optional<std::string>& uid) {
                          ...
                          getMimePayloadAsync(
                              systemBus, path,
                              [tagPath, nfcUid = std::move(nfcUid),
                                         &fruDetails, &nfcState,
                                ...
                                sdbusplus::message_t& message)
{
    ...
    if (ifaceIt != fruDetails.dbusInterfaceMap.end())
    {
        ...
        lg2::info(
            ...
            "UID", uidIt->second.str(), "BUS", fruKeyIt->second.first,
            "ADDR", fruKeyIt->second.second);
    }
...
static void queryNeardObjects(
    ...
    sdbusplus::asio::object_server& objServer)
{
    queryNeardObjectsAsync(systemBus, [systemBus, &fruDetails,
                                       &nfcState, &objServer](
    ...
    std::optional<sdbusplus::bus::match_t>& tagRemovedMatch)
{
    ...
    auto nfcTagAddedHandler =
        [systemBus, &fruDetails, &nfcState,
         &objServer](sdbusplus::message_t& message) {
            handleNfcTagAdded(systemBus, fruDetails, nfcState,
                              objServer, message);
        };
    ...
    auto nfcTagRemovedHandler =
        [&fruDetails, &nfcState,
         &objServer](sdbusplus::message_t& message) {
            handleNfcTagRemoved(fruDetails, nfcState, objServer,
                                message);
        };
