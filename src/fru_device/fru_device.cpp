                              ...
                              off_t offset, std::span<uint8_t> outbuf)
{
    ...
    try
    {
        return fd->read(stdplus::raw::asSpan<std::byte>(outbuf)).size();
    }
    catch (const std::system_error& e)
    {
        return -1;
...
bool i2cFruScan(std::weak_ptr<stdplus::ManagedFd> weakFd, int first, int last,
                int bus, const std::shared_ptr<DeviceMap>& devices,
...
bool writeFRU(uint8_t bus, uint8_t address, std::span<const uint8_t> fru)
{
    ...
    if (hasEepromFile(bus, address))
    {
        ...
        try
        {
            eeprom->write(stdplus::raw::asSpan<std::byte>(fru));
        }
        catch (const std::system_error& e)
        {
