               ...
               sdbusplus::asio::object_server& objServer)
{
    std::future<int> future = std::async(std::launch::async, [&]() {
        ...
        for (const auto& address : skipList)
        {
            std::cout << "skipping " << address << std::endl;
        }
...
std::set<size_t> loadBlocklist(const char* path)
{
    ...
    if (addr == nullptr)
    {
        std::cerr << "addresses must be an array";
        std::exit(EXIT_FAILURE);
    }
    ...
    for (const auto& address : *addr)
    {
        ...
        {
            std::cerr << "address must be a string\n";
            std::exit(EXIT_FAILURE);
        }

        const auto& addrS = address.get_ref<const std::string&>();
        if (!(addrS.starts_with("0x") || addrS.starts_with("0X")))
        {
            std::cerr << "address must start with 0x or 0X\n";
            std::exit(EXIT_FAILURE);
        }

        if (addrS.length() <= 2)
        {
            std::cerr << "malformed address: " << addrS << std::endl;
            std::exit(EXIT_FAILURE);
        }
        ...
        if (erc)
        {
            std::cerr << std::format("Invalid address type: {} {}\n", addrS,
                                     erc.message());
            std::exit(EXIT_FAILURE);
        }
        if (addressInt > 0x77)
        {
            std::cerr << "Invalid address " << addrS << std::endl;
            std::exit(EXIT_FAILURE);
        }
    ...
    std::shared_ptr<sdbusplus::asio::connection>& systemBus)
{
    ...
    auto scan = std::make_shared<FindDevicesWithCallback>(
        ...
        [busNum, &busmap, &dbusInterfaceMap, &unknownBusObjectCount, &powerIsOn,
         &objServer, &systemBus, &addressBlocklist]() {
            ...
            for (auto& device : *(found->second))
            {
                addFruObjectToDbus(device.second, dbusInterfaceMap,
                                   ...
                                   unknownBusObjectCount, powerIsOn,
                                   addressBlocklist, objServer, systemBus);
            }
        });
    scan->run();
}
...
int main()
{
    ...
    iface->register_method("ReScanBus", [&](uint16_t bus) {
        rescanOneBus(busMap, bus, dbusInterfaceMap, true, unknownBusObjectCount,
                     powerIsOn, addressBlocklist, objServer, systemBus);
    });
