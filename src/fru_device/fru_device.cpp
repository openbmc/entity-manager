                             ...
                             sdbusplus::asio::object_server& objServer)
{
    ...
    auto scan = std::make_shared<FindDevicesWithCallback>(
        ...
        [busNum, &objServer, weakFruDevice]() {
            const std::shared_ptr<FruDevice> capturedFruDevice =
                weakFruDevice.lock();
            ...
            for (auto busIface = capturedFruDevice->dbusInterfaceMap.begin();
                 busIface != capturedFruDevice->dbusInterfaceMap.end();)
            {
                if (busIface->first.first == static_cast<size_t>(busNum))
                {
                    objServer.remove_interface(busIface->second);
                    busIface =
                        capturedFruDevice->dbusInterfaceMap.erase(busIface);
                }
                else
                {
                    busIface++;
                 ...
                 capturedFruDevice->utils.getDevices())
            {
                ...
                capturedFruDevice->addFruObjectToDbus(
                    data, static_cast<uint32_t>(busNum), addr, objServer);
            }
        });
    scan->run();
}
    ...
    timer.async_wait([&](const boost::system::error_code& ec) {
        ...
        auto scan = std::make_shared<FindDevicesWithCallback>(
            ...
            shared_from_this(), [&]() {
                ...
                {
                    addFruObjectToDbus(data, bus, addr, objServer);
                }
            });
        scan->run();
    });
...
int main()
{
    ...
    // add it here
    fruDevice->addressBlocklist = fruDevice->loadBlocklist(blocklistPath);
    ...
    std::function<void(sdbusplus::message_t & message)> eventHandler =
        [&](sdbusplus::message_t& message) {
            ...
            if (findState != values.end())
            {
                ...
                {
                    fruDevice->powerIsOn = true;
                }
            }

            if (fruDevice->powerIsOn)
            {
                fruDevice->rescanBusses(objServer);
            }
        };
