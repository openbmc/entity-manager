    ...
    const std::shared_ptr<stdplus::ManagedFd>& fd, uint16_t address)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        ret = i2cSmbusWriteThenRead(fd, address,
                                    stdplus::raw::asSpan<uint8_t>(v), cur);
        if (ret < 0)
        {
            return std::nullopt;
        }
    ...
    std::enable_shared_from_this<FindDevicesWithCallback>
{
    ...
    void run()
    {
        ...
        for (size_t retryCount = 0; retryCount < maxRetries; retryCount++)
        {
            ...
            lg2::info("Retrying {BUSES} in {DELAY} seconds...", "BUSES", msg,
        ...
        if (!retryI2cBuses.empty() && maxRetries > 0)
        {
            lg2::error(
