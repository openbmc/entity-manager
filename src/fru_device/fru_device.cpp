...
// so they must not collide with valid bus/address pairs.
...
{
    return bus == UINT32_MAX && address == 0;
}
        ...
        auto scan = std::make_shared<FindDevicesWithCallback>(
            i2cBuses, busmap, powerIsOn, objServer, addressBlocklist, [&]() {
                for (auto busIface : dbusInterfaceMap)
                {
                    ...
                    // These are not discovered via I2C and would otherwise be removed
...
int main()
{
    ...
    std::function<void(sdbusplus::message_t & message)> nfcTagHandler =
        [&](sdbusplus::message_t& message) {
        ...
        // Only MIME-type NDEF records are expected to contain FRU payload.
        if (*type != "MIME")
        {
        ...
        getMimePayloadAsync(
            systemBus, path,
            [systemBus, &dbusInterfaceMap, &addressBlocklist, &objServer](std::vector<uint8_t> payload) {
                ...
                // NFC-based FRU is not associated with any physical I2C bus.
                // Use a reserved bus value to avoid conflicts with I2C devices.
                addFruObjectToDbus(payload, dbusInterfaceMap,
                                    nfcBus, nfcAddr,
                                    unknownBusObjectCount, powerIsOn,
                                    addressBlocklist, objServer);
            });
    ...
    sdbusplus::bus::match_t nfcTagMatch = sdbusplus::bus::match_t(
        static_cast<sdbusplus::bus_t&>(*systemBus),
        "type='signal',"
        "sender='org.neard',"
