...
bool writeFruFromFile(uint16_t bus, uint8_t address, const std::string& file)
{
    std::ifstream inputFile(file, std::ios_base::binary);
    if (!inputFile.is_open())
    {
        lg2::error("unable to open fru file {FILE}", "FILE", file);
        return false;
    }
    ...
    if (ec)
    {
        lg2::error("unable to open fru file size");
        return false;
    }
