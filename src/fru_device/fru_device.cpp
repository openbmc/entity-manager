...
int FruDevice::getBusFRUs(
    int file, int first, int last, int bus, std::shared_ptr<DeviceMap> devices,
    ...
    std::enable_shared_from_this<FindDevicesWithCallback>
{
    FindDevicesWithCallback(
        const std::vector<fs::path>& i2cBuses, const bool& powerIsOn,
        sdbusplus::asio::object_server& objServer,
        const std::set<size_t>& addressBlocklist, FruDevice& fruDeviceIn,
    ...
    void run()
    {
        fruDevice.findI2CDevices(_i2cBuses, fruDevice.utils.busMap, _powerIsOn,
    ...
    if (ENABLE_FRU_UPDATE_PROPERTY)
    {
        iface->register_method(
            ...
            [bus, address, &dbusInterfaceMap, &unknownBusObjectCount,
             &powerIsOn, &objServer, addressBlocklist, this](
    ...
    sdbusplus::asio::object_server& objServer)
{
    ...
    auto scan = std::make_shared<FindDevicesWithCallback>(
        i2cBuses, powerIsOn, objServer, addressBlocklist, *this,
        [busNum, &dbusInterfaceMap, &unknownBusObjectCount, &powerIsOn,
         &objServer, &addressBlocklist, this]() {
