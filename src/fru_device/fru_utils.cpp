...
static bool updateAreacksum(std::vector<uint8_t>& fruArea)
{
    ...
    fruArea[fruArea.size() - 1] = 0; // Reset checksum byte to 0 before recalculating
...
bool disassemble_fruData(std::vector<uint8_t>& fruData, std::vector<std::vector<uint8_t>>& areasData)
{
    if (fruData.size() < 8)
    {
        std::cerr << "FRU data is too small to contain a valid header.\n";
        ...
        area <= fruAreas::fruAreaMultirecord; ++area)
    {
        ...
        size_t areaSize = fruData[areaOffset + 1] * fruBlockSize; // Area size in bytes
...
bool assemble_fruData(std::vector<uint8_t>& fruData, const std::vector<std::vector<uint8_t>>& areasData)
{
    ...
    for(fruAreas area = fruAreas::fruAreaInternal;
        area <= fruAreas::fruAreaMultirecord; ++area)
    {
        uint8_t areaBlockSize = (areasData[static_cast<size_t>(area)].size() + 
                                fruBlockSize - 1) / fruBlockSize; // Calculate block size
        ...
        fruData.resize(writeOffset + areaBlockSize * fruBlockSize, 0); // Resize to accommodate the area data
...
{
    uint8_t numOfFields = 0, numOfBlocks = 0;
    ...
    {
        areaData.push_back(fieldType); // Field type and length (EndOfFields marker)
    ...
    numOfBlocks = (areaData.size() + fruBlockSize - 1) / fruBlockSize; // Calculate number of blocks needed
