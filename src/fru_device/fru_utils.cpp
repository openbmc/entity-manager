...
bool setField(const fruAreas& fruAreaToUpdate,
                          ...
                          const std::string& propertyName, const std::string& value)
{
    ...
    uint8_t numOfBlocks = (areaData.size() + fruBlockSize - 1) /
                  fruBlockSize;
                     ...
                     const std::vector<std::vector<uint8_t>>& areasData)
{
         ...
         area <= fruAreas::fruAreaMultirecord; ++area)
    {
        ...
        fruData[getHeaderAreaFieldOffset(area)] = (writeOffset + fruBlockSize - 1) / fruBlockSize;
