                  ...
                  uint8_t len)
{
         ...
         area <= fruAreas::fruAreaMultirecord; ++area)
    {
        ...
        {
            lg2::error("{T1} offset is overlapping with {T2} offset", "T1",
                       getFruAreaName(currentArea), "T2", getFruAreaName(area));
            return false;
        }
    }
    return true;
    ...
    FRUReader& reader, const std::string& errorHelp)
{
    ...
    if (hasMultiRecords)
    {
        ...
        while (areaOffset < std::numeric_limits<uint16_t>::max())
        {
            ...
            {
                lg2::error("failed to read {T1} base offset {OFFSET}", "T1",
                           errorHelp, "OFFSET", baseOffset);
                return {{}, true};
            }
...
bool getFruData(std::vector<uint8_t>& fruData, uint32_t bus, uint32_t address)
{
    ...
    {
        lg2::error("Failure getting FRU Info: {ERR}", "ERR", e.what());
        return false;
    }
