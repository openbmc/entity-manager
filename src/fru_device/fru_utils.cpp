...
static bool updateHeadercksum(std::vector<uint8_t>& fruData)
{
    ...
    {
        lg2::debug("FRU header checksum updated from {OLD_CHECKSUM} to {NEW_CHECKSUM}", 
               "OLD_CHECKSUM", static_cast<int>(oldcksum),
               "NEW_CHECKSUM", static_cast<int>(checksum));
    }
    return true;
}
...
static bool updateAreacksum(std::vector<uint8_t>& fruArea)
{
    ...
    {
        lg2::debug("FRU area checksum updated from {OLD_CHECKSUM} to {NEW_CHECKSUM}",
               "OLD_CHECKSUM", static_cast<int>(oldcksum),
               "NEW_CHECKSUM", static_cast<int>(fruArea[fruArea.size() - 1]));
    }
    return true;
}
                     ...
                     const std::vector<std::vector<uint8_t>>& areasData)
{
         ...
         area <= fruAreas::fruAreaMultirecord; ++area)
    {
        ...
        {
            lg2::debug("Skipping empty area: {AREA}", "AREA", getFruAreaName(area));
            continue; // Skip areas that are not present
        }
...
bool createDummyArea(fruAreas fruArea, std::vector<uint8_t>& areaData)
{
    ...
    switch (fruArea)
    {
        ...
        case fruAreas::fruAreaProduct:
            ...
            break;
        default:
            lg2::debug("Invalid FRU area to create: {AREA}", "AREA", static_cast<int>(fruArea));
            return false;
    }
