...
std::map<std::string, std::string>
    parseJSONInit(const nlohmann::json& config)
{
...
void initHwmonDevice(
    const std::string& busPath, uint64_t bus, uint64_t address,
                  ...
                  const nlohmann::json& configuration)
{
    ...
    auto deviceExistsFn =
        std::bind_front(deviceIsCreated, busPath, *bus, *address, hasHWMonDir);
    auto createDeviceFn = std::bind_front(createDevice, busPath, parameters,
                                          std::move(constructor));
    auto deleteDeviceFn =
        std::bind_front(deleteDevice, busPath, *address, std::move(destructor));
