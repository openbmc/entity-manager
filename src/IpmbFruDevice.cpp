    ...
    for (auto& property : formattedFRU)
    {
        ...
        if (property.first == "PRODUCT_ASSET_TAG")
        {
            ...
            iface->register_property(
                ...
                [bus, address, propertyName, &dbusInterfaceMap, &systemBus,
                 ...
                 std::shared_ptr<sdbusplus::asio::connection>& conn)
{
    ...
    for (const auto& pathPair : resp)
    {
        for (const auto& entry : pathPair.second)
        {
            ...
            typeConfig = loadVariant<std::string>(entry.second, "type");
    ...
    for (auto& devicemap : busMap)
    {
        for (auto& device : *devicemap.second)
        {
            AddFRUObjectToDbus(device.second, dbusInterfaceMap, devicemap.first,
                               device.first, systemBus, objServer);
        }
    }
}
