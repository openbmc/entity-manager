...
void findIpmbDev(std::vector<uint8_t>& ipmbAddr)
{
    ...
    try
    {
        ...
        for (const auto& channelConfig : data["Exposes"])
        {
            const std::string& EmType = channelConfig["Type"];

            if (EmType.compare("IPMIChannels") == 0)
            {
                const std::string& typeConfig = channelConfig["type"];
                devIndex = (static_cast<uint8_t>(channelConfig["Index"]));
