                  ...
                  boost::container::flat_map<std::string, std::string>& result)
{
    ...
    else
    {
        ...
        if (*iter == ComponentID::BIC)
        {
            result["MULTIRECORD_OEM_ID1"] =
                std::to_string(static_cast<int>(*iter));
            result["MULTIRECORD_OEM_COMPONENT1"] = "BIC";
        }
        if (*iter == ComponentID::CPLD)
        {
            result["MULTIRECORD_OEM_ID2"] =
                std::to_string(static_cast<int>(*iter));
            result["MULTIRECORD_OEM_COMPONENT2"] = "CPLD";
        }
        uint8_t len = *(++iter);
        if (iter + len > end)
        {
