...
void formatFRUFind(std::vector<uint8_t>& device, boost::container::flat_map<std::string, std::string>& formattedFRU, 
                  uint32_t bus, uint32_t address, size_t& unknownBusObjectCount, std::string &productName)
{
                         ...
                         std::string &productName, int ipmbIndex)
{
    ...
    if (bus > 0)
    {
	    ...
	    for (auto const& busIface : dbusInterfaceMap)
	    {
		    ...
		    {      
			    if(ipmbIndex == -1)    // This condition is only true for I2C based FruDevice
			    {
        ...
        if (property.first == "PRODUCT_ASSET_TAG")
        {
            ...
            iface->register_property(
                ...
                [bus, address, propertyName, &dbusInterfaceMap,
                 ...
                 &systemBus, ipmbFlag](const std::string& req, std::string& resp) {
...
void addFruObjectToDbus(
    ...
    uint32_t bus, uint32_t address, size_t& unknownBusObjectCount,
    const bool& powerIsOn, sdbusplus::asio::object_server& objServer,
    std::shared_ptr<sdbusplus::asio::connection>& systemBus, int ipmbIndex, bool ipmbFlag)
{
    std::cerr << "ipmbIndex : " << ipmbIndex <<std::endl;
    std::cerr << "ipmbFlag : " << ipmbFlag <<std::endl;

    boost::container::flat_map<std::string, std::string> formattedFRU;
    std::string productName;

    formatFRUFind(device, formattedFRU, bus, address, unknownBusObjectCount, productName);
    ...
    searchFRUDbusObjects(bus, address, dbusInterfaceMap, productName, ipmbIndex);


    std::shared_ptr<sdbusplus::asio::dbus_interface> iface;
    if(ipmbFlag)
    {
        iface = objServer.add_interface(productName, "xyz.openbmc_project.Ipmb.FruDevice");
    }
    else
    {
    ...
    findUpdateFRUProperty(formattedFRU, dbusInterfaceMap, bus, address, unknownBusObjectCount, powerIsOn,
                          objServer, systemBus, ipmbFlag, iface); 
