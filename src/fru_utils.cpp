    ...
    std::string& productName, int ipmbIndex)
{
    ...
    if (bus > 0)
    {
        ...
        for (auto const& busIface : dbusInterfaceMap)
        {
            ...
            {
                if (ipmbIndex ==
                    -1) // This condition is only true for I2C based FruDevice
                {
    ...
    bool ipmbFlag)
{
    ...
    std::optional<std::string> optionalProductName = formatFRUFind(device, formattedFRU, bus, address, unknownBusObjectCount);
    ...
    findUpdateFRUProperty(formattedFRU, dbusInterfaceMap, bus, address,
                          unknownBusObjectCount, powerIsOn, objServer,
                          systemBus, ipmbFlag, iface);
