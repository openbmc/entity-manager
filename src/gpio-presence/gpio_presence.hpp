...
{
  public:
    explicit GPIOPresence(sdbusplus::async::context& io,
                          IGPIOProvider& gpioProvider,
                          ConfigProvider& configProvider);

    // NOLINTBEGIN
    sdbusplus::async::task<> startup(sdbusplus::async::context& io,
                                     ConfigProvider& configProvider);
    // NOLINTEND

    // adds a new configuration to our map
    // @param[in] objStr      object path for the new configuration
    // @param[in] config      new configuration
    void addConfig(const std::string& objStr, std::unique_ptr<Config> config);

    // sets up our dbus name
    ...
    // run the gpio event monitor loop
    sdbusplus::async::task<> startGPIOEventMonitor();
