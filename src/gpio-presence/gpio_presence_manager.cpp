    ...
    -> sdbusplus::async::task<void>
{
    ...
    while (!ctx.stop_requested())
    {
        ...
        // event_read() does not clear the EPOLLIN flag immediately; it is cleared during
        // the subsequent epoll check. Therefore, call event_wait with a zero timeout to
        // ensure that event_read() is only invoked when an event is available, preventing
        // it from blocking.
