...
int main()
{
    ...
    std::string matchStr;
    if (entries.type() != nlohmann::json::value_t::array)
    {
        std::cerr << "Invalid contents for nameOwnerChangedMatches field\n";
    }
    else
    {
        for (const auto& entry : entries)
        {
            if (entry.type() != nlohmann::json::value_t::string)
            {
                continue;
            }
            std::string entryStr = entry.get<std::string>();
            if (entryStr.empty())
            {
            ...
            {
                matchStr = sdbusplus::bus::match::rules::nameOwnerChanged(entryStr);
            }
            auto match = std::make_unique<sdbusplus::bus::match::match>(
                static_cast<sdbusplus::bus::bus&>(*systemBus), matchStr,
                [&](sdbusplus::message::message&) {
                    std::cerr << "Tbnguyen nameOwnerChanged " << std::endl;
                    propertiesChangedCallback(systemConfiguration, objServer);
                });
    ...
    if (entries.type() != nlohmann::json::value_t::array)
    {
        std::cerr << "Invalid contents for interfacesAddedMatches field\n";
    }
    else
    {
        for (const auto& entry : entries)
        {
            ...
            {
                matchStr = sdbusplus::bus::match::rules::interfacesAdded(entryStr);
            }
            auto match = std::make_unique<sdbusplus::bus::match::match>(
                static_cast<sdbusplus::bus::bus&>(*systemBus), matchStr,
                [&](sdbusplus::message::message&) {
                    std::cerr << "Tbnguyen interfacesAdded " << std::endl;
                    propertiesChangedCallback(systemConfiguration, objServer);
                });
            matches.emplace_back(std::move(match));
        }
    ...
    if (entries.type() != nlohmann::json::value_t::array)
    {
        std::cerr << "Invalid contents for interfacesRemovedMatches field\n";
    }
    else
    {
        for (const auto& entry : entries)
        {
            ...
            {
                matchStr = sdbusplus::bus::match::rules::interfacesRemoved(entryStr);
            }
            auto match = std::make_unique<sdbusplus::bus::match::match>(
                static_cast<sdbusplus::bus::bus&>(*systemBus), matchStr,
                [&](sdbusplus::message::message&) {
                    std::cerr << "Tbnguyen interfacesRemoved " << std::endl;
                    propertiesChangedCallback(systemConfiguration, objServer);
                });
            matches.emplace_back(std::move(match));
        }
