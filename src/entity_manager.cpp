...

{
    ...
    for (const auto& [boardId, boardConfig] : newConfiguration.items())
    {
        ...
        std::shared_ptr<sdbusplus::asio::dbus_interface> inventoryIface =
            interface::createInterface(objServer, boardPath,
        ...
        interface::createAddObjectMethod(jsonPointerPath, boardPath,
                                         systemConfiguration, objServer,
                                         boardNameOrig);

        interface::populateInterfaceFromJson(
        ...
        for (const auto& [propName, propValue] : boardValues.items())
        {
            if (propValue.type() == nlohmann::json::value_t::object)
            {
                std::shared_ptr<sdbusplus::asio::dbus_interface> iface =
                    interface::createInterface(objServer, boardPath, propName,
                                               boardNameOrig);
            ...
            for (const auto& [name, config] : item.items())
            {
                ...
                if (config.type() == nlohmann::json::value_t::object)
                {
                    ...
                    std::shared_ptr<sdbusplus::asio::dbus_interface>
                        objectIface = interface::createInterface(
                               ...
                               const nlohmann::json& device)
{
    ...
    auto& ifaces = interface::getDeviceInterfaces(device);
...
int main()
{
    ...
    std::set<std::string> probeInterfaces = interface::getProbeInterfaces();
    ...
    interface::tryIfaceInitialize(entityIface);
