...
int main()
{
    ...
    else
    {
        for (const auto& entry : entries)
        {
            ...
            {
                matchStr =
                    sdbusplus::bus::match::rules::nameOwnerChanged(entryStr);
            }
    ...
    else
    {
        for (const auto& entry : entries)
        {
            ...
            {
                matchStr =
                    sdbusplus::bus::match::rules::interfacesAdded(entryStr);
            }
            auto match = std::make_unique<sdbusplus::bus::match::match>(
                static_cast<sdbusplus::bus::bus&>(*systemBus), matchStr,
                [&](sdbusplus::message::message&) {
    ...
    else
    {
        for (const auto& entry : entries)
        {
            ...
            {
                matchStr =
                    sdbusplus::bus::match::rules::interfacesRemoved(entryStr);
            }
            auto match = std::make_unique<sdbusplus::bus::match::match>(
                static_cast<sdbusplus::bus::bus&>(*systemBus), matchStr,
                [&](sdbusplus::message::message&) {
