    ...
    std::shared_ptr<sdbusplus::asio::connection>& systemBus)
{
    ...
    if (!fruAreaUpdate(fruData, propertyName))
    {
        std::cerr << "fruAreaUpdate failed \n";
        return false;
    }

    // Push post update fru field bytes to a vector
    fruAreaParams.fieldLength =
        getFieldLength(fruData[fruAreaParams.fruUpdateFieldLoc]);
    ...
    size_t restFRUFieldsLoc = fruAreaParams.fruDataIter;
    size_t endOfFieldsLoc = 0;
    while ((fruAreaParams.fieldLength =
    ...
    endOfFieldsLoc = fruAreaParams.fruDataIter;
    ...
    size_t fruAreaDataSize =
        ((fruAreaParams.fruUpdateFieldLoc - fruAreaParams.fruAreaStart + 1) +
         restFRUAreaFieldsData.size());
    size_t fruAreaAvailableSize = fruAreaParams.fruAreaSize - fruAreaDataSize;
    if ((updatePropertyReqLen + 1) > fruAreaAvailableSize)
    {
        ...
        size_t newFRUDataSize =
            fruData.size() + newFRUAreaSize - fruAreaParams.fruAreaSize;
        fruData.resize(newFRUDataSize);
        fruAreaParams.fruAreaSize = newFRUAreaSize;
    ...
    fruData[fruAreaParams.fruUpdateFieldLoc] =
        static_cast<uint8_t>(updatePropertyReqLen | newTypeLenMask);
    fruAreaParams.fruUpdateFieldLoc++;
    std::copy(updatePropertyReq.begin(), updatePropertyReq.end(),
              fruData.begin() + fruAreaParams.fruUpdateFieldLoc);

    // Copy remaining data to main fru area - post updated fru field vector
    restFRUFieldsLoc = fruAreaParams.fruUpdateFieldLoc + updatePropertyReqLen;
    ...
    unsigned int nextFRUAreaNewLoc =
        updateFRUAreaLenAndChecksum(fruData, fruAreaParams.fruAreaStart,
                                    fruAreaDataEnd, fruAreaParams.fruAreaEnd);
