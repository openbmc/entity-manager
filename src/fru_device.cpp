...
static int64_t readData(bool is16bit, int file, uint16_t address,
                             off_t offset, size_t len, uint8_t* buf, bool bytewise)
{
    if (!is16bit)
    {
          if(!bytewise)
          ...
          for(size_t i=0; i < len; i++){
                ...
                int byte = i2c_smbus_read_byte_data(file, static_cast<uint8_t>(offset+i));
                if(byte < 0) return static_cast<int64_t>(byte);
...
static std::vector<uint8_t> processEeprom(int bus, int address)
{
    ...
    std::vector<uint8_t> device = readFRUContents(reader, errorMessage, std::nullopt);
               ...
               sdbusplus::asio::object_server& objServer)
{
    std::future<int> future = std::async(std::launch::async, [&]() {
        ...
        for (int ii = first; ii <= last; ii++)
        {
            ...
            bool foundHeader;
            std::vector<uint8_t> device = readFRUContents(reader, errorMessage, std::optional<bool*>{&foundHeader});
            ...
            if(!foundHeader){
                ...
                device = readFRUContents(readerBytewise, errorMessage, std::nullopt);
            }
