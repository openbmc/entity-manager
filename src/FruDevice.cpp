            ...
            if (deviceIs16Bit < 0)
            {
                ...
                if (powerIsOn)
                {
                    ...
                    else
                    {
                        ...
                        status.is16Bit = 0;
                        status.rootFail = 1;
                        devItems.insert(std::pair<size_t, AddressStatus>(ii, status));
                    }
                }
                continue;
            }
...
void loadDevice16BitList(const std::filesystem::path& path)
{
    ...
    for (const nlohmann::json& listConfigs : device16bitcfg)
    {
        ...
        auto bus = listConfigs["bus"];
        if (bus.type() != nlohmann::json::value_t::number_unsigned)
        {
        ...
        auto dev16BitAddresses = listConfigs["address"];
        ...
        try
        {
            for (const auto& addr : dev16BitAddresses)
            {
                ...
                else
                {
                    ...
                    status.is16Bit = 1;
                    status.rootFail = 0;
                    addrItems.insert(std::pair<size_t, AddressStatus>(addr, status));
                }
            }
        }
        catch (const nlohmann::detail::type_error& e)
        {
            // Type mis-match is a critical error.
            std::cerr << "Invalid address type when load eeprom16Bitlist json: "
                << e.what() << "\n";
            std::exit(EXIT_FAILURE);
        }
    }
