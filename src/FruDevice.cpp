...
uint64_t I2C_eeprom_nic(std::shared_ptr<sdbusplus::asio::connection>& conn,
                        std::string name)
{
    uint64_t result;
               ...
               std::shared_ptr<sdbusplus::asio::connection>& systemBus)
{
    ...
    int flag;
    std::string eepromName = "Class";
    std::string eepromAddr = "Address";
    auto eeprom = I2C_eeprom_nic(systemBus, eepromName);
    ...
    std::future<int> future = std::async(std::launch::async, [&]() {
        ...
        for (int ii = first; ii <= last; ii++)
        {
            ...
            /* Check for Device type for NIC card, if it is 8 bit or 16 bit */
            auto addr = I2C_eeprom_nic(systemBus, eepromAddr);
            if (addr == 81)
            {
                flag = eeprom;
            }
