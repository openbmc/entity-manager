...
{
    auto deviceMap = fruUtilObj.busMap.find(bus);
    if (deviceMap == fruUtilObj.busMap.end())
    {
...
int main()
{
    ...
    iface->register_method("ReScan", [&]() {
        rescanBusses(fruUtilObj.busMap, dbusInterfaceMap, unknownBusObjectCount, powerIsOn,
                     objServer, systemBus);
    });

    iface->register_method("ReScanBus", [&](uint8_t bus) {
        rescanOneBus(fruUtilObj.busMap, bus, dbusInterfaceMap, true, unknownBusObjectCount,
    ...
    std::function<void(sdbusplus::message::message & message)> eventHandler =
        [&](sdbusplus::message::message& message) {
            ...
            if (powerIsOn)
            {
                rescanBusses(fruUtilObj.busMap, dbusInterfaceMap, unknownBusObjectCount,
                             powerIsOn, objServer, systemBus);
            }
        };
                ...
                switch (iEvent->mask)
                {
                    ...
                    case IN_DELETE:
                        ...
                        if (boost::starts_with(name, "i2c"))
                        {
                            ...
                            rescanOneBus(fruUtilObj.busMap, static_cast<uint8_t>(bus),
