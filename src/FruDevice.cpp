...
static constexpr std::array<const char*, FRU_AREA_COUNT> FRU_AREA_NAMES = {
...
static inline unsigned int getHeaderAreaFieldOffset(int field)
{
    return static_cast<unsigned int>(field + 1);
}
                                     ...
                                     const std::string& errorHelp)
{
    ...
    for (int jj = 0; jj < FRU_AREA_COUNT; jj++)
    {
        // Offset value can be 255.
        unsigned int areaOffset = device[getHeaderAreaFieldOffset(jj)];
        if (areaOffset == 0)
        {
            continue;
        }
    ...
    if (hasMultiRecords)
    {
        ...
        unsigned int areaOffset =
            device[getHeaderAreaFieldOffset(FRU_AREA_MULTIRECORD)];
        areaOffset *= 8;
               ...
               boost::container::flat_map<std::string, std::string>& result)
{
    ...
    for (int area = FRU_AREA_CHASSIS; area <= FRU_AREA_PRODUCT; area++)
    {
        ...
        do
        {
            ...
            {
                name = std::string(FRU_AREA_NAMES[area]) + "_" +
                       fruAreaFieldsNames->at(fieldIndex);
            }
            else
            {
                name =
                    std::string(FRU_AREA_NAMES[area]) + "_" +
                    FRU_CUSTOM_FIELD_NAME +
                    std::to_string(fieldIndex - fruAreaFieldsNames->size() + 1);
            }
        ...
        std::shared_ptr<sdbusplus::asio::dbus_interface>>& dbusInterfaceMap)
{
    ...
    int fruAreaToUpdate = it - FRU_AREA_NAMES.begin();
    fruAreaOffsetFieldValue =
        fruData[getHeaderAreaFieldOffset(fruAreaToUpdate)];
    propertyNamePrefix = areaName + "_";
    ...
    for (int nextFRUArea = FRU_AREA_INTERNAL;
         nextFRUArea <= FRU_AREA_MULTIRECORD; nextFRUArea++)
    {
    ...
    if (nextFRUAreaOffsetDiff && nextFRUAreaLoc)
    {
        ...
        for (int fruArea = FRU_AREA_INTERNAL; fruArea <= FRU_AREA_MULTIRECORD;
             fruArea++)
        {
