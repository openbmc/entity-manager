                                     ...
                                     const std::string& errorHelp)
{
    ...
    for (int area = fruAreaInternal; area <= fruAreaMultirecord; area++)
    {
    ...
    if (hasMultiRecords)
    {
        ...
        unsigned int areaOffset =
            device[getHeaderAreaFieldOffset(fruAreaMultirecord)];
        areaOffset *= fruBlockSize;
               ...
               boost::container::flat_map<std::string, std::string>& result)
{
    ...
    for (int area = fruAreaChassis; area <= fruAreaProduct; area++)
    {
        ...
        if (calculateChecksum(fruBytes.begin() + offset, fruBytesIterEndArea) !=
            *fruBytesIterEndArea)
        {
            std::cerr << "Checksum error in FRU area " << FRU_AREA_NAMES[area]
                      << "\n";
            return false;
        }
        ...
        switch (area)
        {
            ...
            case fruAreaBoard:
            {
                ...
                // If Lang is not English encoding defined to be 2-byte UNICODE,
                // but we don't handle this case
                if (lang && lang != 25)
                {
                    std::cerr << "Warning: language other then English not "
                                 "supported \n";
                }
                fruBytesIter += 1;
            ...
            else
            {
                if (fieldIndex < fruAreaFieldNames->size())
                {
                    std::cerr << "Mandatory fields absent in FRU area "
                              << FRU_AREA_NAMES[area] << " after " << name
                              << "\n";
                }
            }
        ...
        std::shared_ptr<sdbusplus::asio::dbus_interface>>& dbusInterfaceMap)
{
    ...
    for (int nextFRUArea = fruAreaInternal; nextFRUArea <= fruAreaMultirecord;
         nextFRUArea++)
    {
