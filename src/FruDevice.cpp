...
std::set<int> findI2CEeproms(int i2cBus, std::shared_ptr<DeviceMap> devices)
{
    ...
    // NOTE: This should be faster than just checking for each possible address path.
    for (const auto& p : fs::directory_iterator(path)) {
        const std::string node = p.path().string();
        std::smatch m;
        bool found = std::regex_match(node, m, std::regex(".+\\d+-([0-9abcdef]+$)"));
        ...
        // There is an eeprom file at this address, it may have invalid contents, but we found it.
                 ...
                 std::shared_ptr<DeviceMap> devices)
{
    ...
    std::future<int> future = std::async(std::launch::async, [&]() {
        ...
        // NOTE: When reading the devices raw on the bus, it can interfere with the driver's ability to operate, therefore read eeproms first before scanning for devices without drivers.
        // Several experiments were run and it was determined that if there were any devices on the bus before the eeprom was hit and read, the eeprom driver wouldn't open while the bus device was open.
        // An experiment was not performed to see if this issue was resolved if the i2c bus device was closed, but hexdumps of the eeprom later were successful.
