...
{
    val &= 0xf;
    return (val < 10) ? static_cast<char>(val + '0') : bcd_high_chars[val - 10];
}
    ...
    switch (type)
    {
        ...
        case 0x1:
            ...
            for (i = 0; i < len; i++, iter++)
            {
                ...
                value.push_back(bcdplus_to_char(val >> 4));
                value.push_back(bcdplus_to_char(val & 0xf));
            }
            break;
        ...
        case 0x2:
            /* 6-bit ASCII */
            {
                unsigned int accum, accum_bitlen = 0;
                value = std::string();
                for (i = 0; i < len; i++, iter++)
                {
                    /* we need to cast to unsigned before the promotion to
                     * int, so we don't sign-extend. */
                    accum |= static_cast<unsigned char>(*iter) << accum_bitlen;
                    accum_bitlen += 8;
