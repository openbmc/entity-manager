    ...
    sdbusplus::asio::object_server& objServer)
{
    ...
    timer.async_wait([&](const boost::system::error_code& ec) {
        ...
        auto busMatchString = std::string(R"(\w[^-]*$)");
        if (!findFilesWithMap(devDir, matchString, busMatchString, busPaths))
        {
            std::cerr << "unable to find i2c devices\n";
            return;
        }

        for (auto it = busPaths.begin(); it != busPaths.end(); ++it)
        {
            i2cBuses.emplace_back(it->second);
        }
