...
class fruDevice
{
    ...
    int readBlockEEPROM(uint16_t offset, uint8_t len, uint8_t* buf)
    {
        ...
        {
            std::cerr << "EEPROM (" << fruBus << "," << fruAddress
    ...
    {
        return i2c_smbus_read_i2c_block_data(externalBusFD.value(),
                                             offset ^ 0xff, len, buf);
    }
    ...
    int readBlock16Bit(uint16_t offset, uint8_t len, uint8_t* buf)
    {
        ...
        return i2c_smbus_read_i2c_block_data(externalBusFD.value(), highAddr,
                                             len, buf);
    }
  ...
  public:
    fruDevice(int bus, int addr) : fruBus(bus), fruAddress(addr), is16Bit(false)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    std::cerr << "Setting slave address: " << addr << " on bus "
                              << bus << " failed" << std::endl;
                ...
                else
                {
                    ...
                    {
                        is16Bit = bool(flag);
                        std::cout << bus << addr << " is16bit: " << is16Bit
                                  << std::endl;
                    }
                }
            }
        ...
        for (int ii = first; ii <= last; ii++)
        {
            ...
            if (sum == block_data[7])
            {
                ...
                for (int jj = 1; jj <= FRU_AREAS.size(); jj++)
                {
                    ...
                    {
                        area_offset *= 8;
                        if (dev.readBlock(area_offset, 0x8, block_data.data()) <
                            0)
                        {
