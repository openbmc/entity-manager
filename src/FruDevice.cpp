...
using ReadBlockFunc = std::function<int64_t(int flag, int file, uint16_t address,
...
std::vector<char> readFruContents(int flag, int file, uint16_t address, ReadBlockFunc readBlock,
                                  const std::string& errorHelp)
{
    ...
    for (size_t jj = 1; jj <= FRU_AREAS.size(); jj++)
    {
        ...
        if (readBlock(flag, file, address, static_cast<uint16_t>(areaOffset), 0x2,
                      blockData.data()) < 0)
        {
            std::cerr << "failed to read " << errorHelp << "\n";
            return {};
    ...
    if (hasMultiRecords)
    {
        ...
        while (areaOffset < std::numeric_limits<uint16_t>::max())
        {
            ...
            if (readBlock(flag, file, address, static_cast<uint16_t>(areaOffset), 0x3,
                          blockData.data()) < 0)
            {
                std::cerr << "failed to read " << errorHelp << "\n";
                return {};
...
// Issue an I2C transaction with the format of writing to_slave_buf_len bytes
// of data, restart, then reading from_slave_buf_len bytes of data.
static int i2c_smbus_write_then_read(int file, uint16_t address, uint8_t *to_slave_buf, uint8_t to_slave_buf_len,
                             uint8_t *from_slave_buf, uint8_t from_slave_buf_len)
{
  if (to_slave_buf == NULL || to_slave_buf_len == 0 || from_slave_buf == NULL || from_slave_buf_len == 0 ) {
    return -1;
  }
...
static int64_t readBlockData(int flag, int file, uint16_t address, uint16_t offset, uint8_t len,
                             uint8_t* buf)
{
    if (flag == 0)
    {
        return i2c_smbus_read_i2c_block_data(file, static_cast<uint8_t>(offset), len, buf);
    }

    offset = htobe16(offset);
    return i2c_smbus_write_then_read(file, address, reinterpret_cast<uint8_t *>(&offset), 2, buf, len);
}
...
static std::vector<char> processEeprom(int bus, int address)
{
    ...
    std::vector<char> device =
        readFruContents(0, file, static_cast<uint16_t>(address), readFromEeprom, errorMessage);
               ...
               std::shared_ptr<DeviceMap> devices)
{
    ...
    std::future<int> future = std::async(std::launch::async, [&]() {
        ...
        for (int ii = first; ii <= last; ii++)
        {
            ...
            std::vector<char> device =
                readFruContents(flag, file, static_cast<uint16_t>(ii), readBlockData, errorMessage);
            if (device.empty())
            {
                continue;
            }
