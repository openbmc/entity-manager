...
static std::string getEepromPath(size_t bus, size_t address)
{
    ...
    output << "/sys/bus/i2c/devices/" << bus << "-" << std::right << std::setfill('0') << std::setw(4) << std::hex << address << "/eeprom";
    return output.str();
}
...
static ssize_t readFromEeprom(int fd, uint16_t offset, uint8_t len, uint8_t* buf)
{
...
// TODO: This code is very similar to the non-eeprom version and can be merged with some tweaks.
static std::vector<char> processEeprom(int bus, int address)
{
    ...
    if (readBytes < 0) {
        std::cerr << "failed to read eeprom at " << bus << " address " << address
                  << "\n";
        close(file);
        return device;
    }
    ...
    for (size_t jj = 1; jj <= FRU_AREAS.size(); jj++)
    {
        ...
        while (length > 0) {
            ...
            if (readFromEeprom(file, area_offset, static_cast<uint8_t>(to_get), block_data.data()) < 0)
            {
            ...
            device.insert(device.end(), block_data.begin(), block_data.begin() + to_get);
                 ...
                 std::shared_ptr<DeviceMap> devices)
{
    ...
    std::future<int> future = std::async(std::launch::async, [&]() {
        ...
        for (int ii = first; ii <= last; ii++)
        {
            ...
            if (hasEepromFile(bus, ii))
            {
                device = processEeprom(bus, ii);
                if (!device.empty()) {
