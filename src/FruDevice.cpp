...
{
    size_t sum = 0;
    size_t checksumLoc, endLengthLoc = 0, traverseProdInfoAreaLoc = 1;
    ...
    if (!mod)
    {
        for (size_t i = 1; i < prodAreaLenMultipleBytes; i++)
        {
    ...
    {
        for (size_t i = 1; i < (prodAreaLenMultipleBytes - mod); i++)
        {
            fruData[++endLengthLoc] = 0;
            traverseProdInfoAreaLoc++;
        }
...
static size_t getLocationFromTypeLength(size_t indexToAssetTag,
                                        std::vector<uint8_t> fruData)
{
    constexpr uint8_t typeLenMask = 0x3F;
    size_t typeLength = fruData[indexToAssetTag] & typeLenMask;
    return indexToAssetTag + typeLength;
}
...
bool updateAssetTag(const std::string& assetTag, uint32_t bus, uint32_t address)
{
    ...
    size_t assetTagLoc, skipToAssetTag = 5;
    ...
    size_t postAssetTagLoc = getLocationFromTypeLength(assetTagLoc, fruData);
    postAssetTagLoc++;
