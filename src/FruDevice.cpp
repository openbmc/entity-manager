                       ...
                       uint32_t address, std::string propertyName)
{
    ...
    try
    {
        std::vector<uint8_t> fruData = getFruInfo(
            static_cast<uint8_t>(bus), static_cast<uint8_t>(address));
    }
    catch
    {
        std::cerr << "\n Failure getting Fru Info";
        return false;
    }
    ...
    {
        std::cerr << "\n ProperyName doesn't exist in Fru Area Vector";
        return false;
    }
    ...
    {
        std::cerr << "FruData Size increased total Fru bytes";
        throw std::invalid_argument("FruData Size increased total Fru bytes");
    }
