...
// the object on dbus
using DbusInterfaceMap = boost::container::flat_map<
    std::pair<size_t, size_t>,
    std::shared_ptr<sdbusplus::asio::dbus_interface>>;

static DbusInterfaceMap dbusInterfaceMap;
...
bool updateFruProperty(const std::string& assetTag, uint32_t bus,
                       uint32_t address, std::string propertyName);
...
void AddFruObjectToDbus(std::vector<char>& device,
                        DbusInterfaceMap& dbusInterfaceMap, uint32_t bus,
                        uint32_t address)
{
    ...
    for (auto& property : formattedFru)
    {
        ...
        if (property.first == "PRODUCT_ASSET_TAG")
        {
            ...
            iface->register_property(
                ...
                [bus, address, propertyName](const std::string& req,
                                             std::string& resp) {
...
void rescanOneBus(BusMap& busmap, uint8_t busNum,
                  DbusInterfaceMap& dbusInterfaceMap, bool dbusCall)
{
...
void rescanBusses(BusMap& busmap, DbusInterfaceMap& dbusInterfaceMap)
{
...
bool updateFruProperty(const std::string& updatePropertyReq, uint32_t bus,
                       uint32_t address, std::string propertyName)
{
