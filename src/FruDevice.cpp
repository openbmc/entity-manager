...
static const char bcd_high_chars[] = {
    ' ', '-', '.', 'X', 'X', 'X',
};
...
{
    val &= 0xf;
    return (val < 10) ? (val + '0') : bcd_high_chars[val - 10];
}
    ...
    switch (type)
    {
        ...
        case 0x1:
            ...
            for (i = 0; i < len; i++, iter++)
            {
                ...
                value.push_back(bcdplus_to_char(c >> 4));
                value.push_back(bcdplus_to_char(c & 0xf));
            }
            break;
        ...
        case 0x2:
            /* 6-bit ASCII */
            {
                ...
                for (i = 0; i < len; i++, iter++)
                {
                    /* we need to cast to unsigned before uint16_t, so we
                     * don't sign-extend into the top 8 bits */
                    uint16_t val = static_cast<uint16_t>(
                        static_cast<unsigned char>(*iter));
                    accum |= val << accum_bitlen;
                    accum_bitlen += 8;
