...
class FruDevice {
    ...
    FruDevice(int bus_fd, int bus, int addr) : bus_m(bus), addr_m(addr), is16bit(false)
    {
        if (isEEPROM(bus, addr)) {
            ...
            if (flag >= 0) {
                ...
                std::cerr << "Error getting 8/16 bit addressing for bus:" << bus << " addr: " << addr << std::endl;
            }
        }
    }
    bool valid() {
    ...
    int read_block_eeprom(uint16_t offset, uint8_t len, uint8_t *buf) {
        ...
        if (read(eeprom_file_m.value(), buf, len) != len) {
            std::cerr << "EEPROM (" << bus_m << "," << addr_m << " Read failed" << std::endl;
            return -1;
        }
        return 0;
    }
    int read_block_8bit(uint16_t offset, uint8_t len, uint8_t *buf) {
        return i2c_smbus_read_i2c_block_data(bus_file_m.value(), offset ^ 0xff, len, buf);
    }
    int read_block_16bit(uint16_t offset, uint8_t len, uint8_t *buf) {
        ...
        return i2c_smbus_read_i2c_block_data(bus_file_m.value(), high_addr, len, buf);
    }
    ...
    {
        return is_regular_file(std::filesystem::path(
            EEPROMPath(bus, addr)));
    }
