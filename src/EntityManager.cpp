...
const static boost::container::flat_map<std::string, SocketProbeUtility>
    socketProbeTable = {
        {"CPUSocket",
...

{
    ...
    {
        for (const auto& [board, assoList] : boardAssocTable)
        {
            auto it = association.find(board);
            if (it != association.end())
            {
                it->second->register_property("Associations", assoList);
                    ...
                    findBus->type() == nlohmann::json::value_t::number_unsigned)
                {
                    ...
                    if (findLabel != item.end())
                    {
                        ...
                        i2cSlots[findBus->get<uint64_t>()].emplace_back(
                            std::make_tuple(boardName + "/" + itemName,
                                            boardKeyOrig, boardType,
                                            itemLabel));
                    }
                }
            }
    ...
    {
        auto outNode = indexNodeMap[std::get<0>(asso)];
        auto inNode = indexNodeMap[std::get<1>(asso)];
        auto edge = indexEdgeMap[std::get<2>(asso)];

        std::string boardName = std::get<1>(outNode);
        auto objectPath = std::get<0>(inNode);

        // todo(jjy): choose proper forward and reverse types based on the
        // entity board typer. Here assumming all board entities will be
        // reprented by Redfish Chassis Schema. Using containedby and contains
        // assocation type that follows Chassis Schema
        boardAssocTable[boardName].emplace_back("containedby", "contains",
                                                objectPath);
