    ...
    const std::string& prodName)
{
    if (association)
    {
        ...
        associations.emplace_back(boardKey, prodName, path);
...
std::string getIfaceFromJsonPtr(const std::string& jsonPointerString)
{
...
bool isPropertyUpdatable(const std::string& propertyName,
                         const std::string& jsonPointerString)
{
    std::string interface = getIfaceFromJsonPtr(jsonPointerString);
    if (updatablePropertyMap.find(propertyName) != updatablePropertyMap.end())
    {
        if (getPermission(interface) ==
            sdbusplus::asio::PropertyPermission::readWrite)
        {
            return true;
        }
...
bool persistProperty(const std::string& propertyName,
                     const PropertyType& newVal, const std::string& path)
{
    ...
    iface->register_property(
        ...
        [propertyName, &systemConfiguration,
         ...
         iface](const PropertyType& newVal, PropertyType& val) {
            ...
            {
                if (!persistProperty(propertyName, newVal,
    ...
    for (auto& boardPair : newConfiguration.items())
    {
        ...
        for (auto& boardField : boardValues.items())
        {
            ...
            if (boardField.key() == probePath)
            {
                ...
                std::shared_ptr<sdbusplus::asio::dbus_interface> association =
                    objServer.add_interface(boardName, association::interface);
