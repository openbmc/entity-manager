...

{
    ...
    // I2C bus -> {(connector path, entity name,  entity type, location)}
...
#endif // ENABLE_ENTITY_ASSOCIATION_I2C
    ...
    boost::container::flat_map<uint32_t, Node> idxNode;
    boost::container::flat_map<Node, uint32_t> nodeIdx;
    boost::container::flat_map<uint32_t, Edge> idxEdge;
    boost::container::flat_map<Edge, uint32_t> edgeIdx;
    Graph graph;
    for (const auto& i2cEntity : i2cEntities)
    {
        ...
        for (const auto& entity : i2cEntity.second)
        {
            for (const auto& slot : i2cSlots[i2c])
            {
                ...
                {
                    uint32_t inId, outId, eId;
                    if (nodeIdx.find(connectedEntity) != nodeIdx.end())
                    {
                        inId = nodeIdx[connectedEntity];
                    }
                    else
                    {
                        inId = ndx++;
                        nodeIdx.emplace(connectedEntity, inId);
                        idxNode.emplace(inId, connectedEntity);
                    }

                    if (nodeIdx.find(entity) != nodeIdx.end())
                    {
                        outId = nodeIdx[entity];
                    }
                    else
                    {
                        outId = ndx++;
                        nodeIdx.emplace(entity, outId);
                        idxNode.emplace(outId, entity);
                    }

                    if (edgeIdx.find(slot) != edgeIdx.end())
                    {
                        eId = edgeIdx[slot];
                    }
                    else
                    {
                        eId = egx++;
                        edgeIdx.emplace(slot, eId);
                        idxEdge.emplace(eId, slot);
                    }
                    graph[outId].emplace(inId, eId);
                }
            }
        }
    }
    ...
    {
        auto outNode = idxNode[std::get<0>(asso)];
        auto inNode = idxNode[std::get<1>(asso)];
        auto edge = idxEdge[std::get<2>(asso)];

        auto assoIntf =
            createInterface(objServer, std::get<0>(outNode),
                            "xyz.openbmc_project.Association.Definitions",
                            std::get<1>(outNode));
        auto objectPath = std::get<0>(inNode);
        std::vector<Association> assoProperty;

        // todo(jjy): choose proper forward and reverse types based on the
        // entity board type
        assoProperty.emplace_back("containedby", "contains", objectPath);
        assoIntf->register_property("Associations", assoProperty);
        assoIntf->initialize();

        // We use the connector name that denotes silkscreen name as the enitiy
        // location
