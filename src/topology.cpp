...
void Topology::addBoard(std::string path, std::string boardType, const nlohmann::json& exposesItem)
{
    ...
    if (exposesType == "DownstreamPort")
    {
        ...
        {
            std::cerr << "Board at path " << path << " is missing ConnectsToType" << std::endl;
            return;
        }
    ...
    for (const auto& upstreamPortPair : upstreamPorts)
    {
        ...
        for (const Path& upstream : upstreamPortPair.second)
        {
            for (const Path& downstream : downstreamMatch->second)
            {
                ...
                if (boardTypes[downstream] == "Chassis" && boardTypes[upstream] == "Chassis")
                {
                    result[downstream].emplace_back("chassisContainedBy",
                                                    "chassisContains",
                                                    upstream);
                }
            }
