             ...
             const nlohmann::json::array_t& channelNames)
{
    ...
    for (const nlohmann::json& value : channelNames)
    {
        const std::string* channelName = value.get_ptr<const std::string*>();
                  ...
                  const nlohmann::json& configuration)
{
    ...
    else if (boost::ends_with(type, "Mux"))
    {
        std::cerr << configuration.dump() << "\n";
        std::cerr << "bus=" << bus << "\n";
