...
int main()
{
    ...
    std::shared_ptr<sdbusplus::asio::dbus_interface> iface =
        objServer.add_interface(fruDbusObject, fruDbusIface);

    iface->register_method("ReScan", [&]() {
        rescanBuses(busMap, dbusInterfaceMap, objServer, systemBus);
    });

    iface->register_method("ReScanBus", [&](uint8_t bus) {
        rescanOneBus(busMap, bus, dbusInterfaceMap, true, objServer, systemBus);
    });
                    ...
                    case IN_DELETE:
                        ...
                        if (boost::starts_with(name, "i2c"))
                        {
                            ...
                            rescanOneBus(busMap, static_cast<uint8_t>(bus),
                                         dbusInterfaceMap, false, objServer,
                                         systemBus);
                        }
                }
