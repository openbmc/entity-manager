...
size_t findBalancedEnd(std::string_view s, size_t open)
{
    ...
    // Reached the end of input with the outermost '(' still open: depth never
    // returned to zero, e.g. an unbalanced paren or an unterminated quote (a
    // quote keeps depth pinned above zero, so this covers both).
