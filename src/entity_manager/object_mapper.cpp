...
void object_mapper::getSubTree(
    ...
    const int32_t depth, const std::set<std::string>& interfaces,
                       ...
                       const GetSubTreeType& interfaceSubtree)>& callback)
{
    ...
    systemBus.async_method_call(
        callback, ObjectMapper::default_service, ObjectMapper::instance_path,
        ObjectMapper::interface, "GetSubTree", path, depth, interfaces);
}
