    ...
    FoundDevices& foundDevices)
{
    ...
    for (auto itr = foundDevices.begin(); itr != foundDevices.end();)
    {
        ...
        auto record = _em.systemMapper.systemConfiguration.find(recordName);
        if (record == _em.systemMapper.systemConfiguration.end())
        {
            record = _em.systemMapper.lastJson.find(recordName);
            if (record == _em.systemMapper.lastJson.end())
            {
                itr++;
                continue;
            }
            ...
            _em.systemMapper.systemConfiguration[recordName] = *record;
        }
    ...
    for (const auto& [foundDevice, path] : foundDevices)
    {
        ...
        for (auto& expose : *findExpose)
        {
                 ...
                 keyPair++)
            {
                ...
                applyExposeActions(_em.systemMapper.systemConfiguration,
                                   recordName, expose, keyPair);
            }
        }
