    ...
    std::optional<std::string>& replaceStr)
{
    ...
    if (!replaceStr && replaceVal)
    {
        if (usedNames.contains(nameTemplate))
        {
            ...
            em_utils::templateCharReplace("Name", copyForName, dbusObject,
    ...
    if (ret == nullptr)
    {
        lg2::error("Device name wasn't a string???");
        return "";
    }
    return *ret;
}

void scan::PerformScan::updateSystemConfiguration(nlohmann::json& recordRef,
                                                  const std::string& probeName,
                                                  FoundDevices& foundDevices)
{
    ...
    for (const auto& [foundDevice, path] : foundDevices)
    {
        ...
        nlohmann::json::object_t* record =
            recordRef.get_ptr<nlohmann::json::object_t*>();
        if (record == nullptr)
        {
        ...
        for (auto& keyPair : *record)
        {
            if (keyPair.first != "Name")
            {
                ...
                em_utils::templateCharReplace(keyPair.first, keyPair.second,
                                              dbusObject, foundDeviceIdx,
                                              replaceStr, handleLeftOver);
            }
        }
        ...
        nlohmann::json::object_t* exposeObj =
            findExpose->second.get_ptr<nlohmann::json::object_t*>();
        if (exposeObj == nullptr)
        {
            lg2::error("Exposes wasn't a object? {JSON}", "JSON",
                       recordRef.dump());
            continue;
        }

        for (auto& [exposeKey, exposeValue] : *exposeObj)
        {
            nlohmann::json::object_t* exposeObj2 =
                exposeValue.get_ptr<nlohmann::json::object_t*>();
            if (exposeObj2 == nullptr)
            {
                lg2::error("Exposes wasn't a object? {JSON}", "JSON",
                           recordRef.dump());
                continue;
            ...
            for (auto& [key, value] : *exposeObj2)
            {
                em_utils::templateCharReplace(key, value, dbusObject,
                                              foundDeviceIdx, replaceStr);

                applyExposeActions(_em.systemConfiguration, recordName,
                                   *exposeObj2, key, value);
            }
        }
...
void scan::PerformScan::run()
{
    ...
    for (auto it = _configurations.begin(); it != _configurations.end();)
    {
        ...
        nlohmann::json::array_t* probeCommandPtr =
            findProbe->get_ptr<nlohmann::json::array_t*>();
        if (probeCommandPtr == nullptr)
        {
            for (const auto& probe : *probeCommandPtr)
            {
