...
void scan::PerformScan::applyExposes(
    const std::string& recordName, nlohmann::json& expose,
...
void scan::PerformScan::addRecordProbePath(
    const nlohmann::json::object_t& record, const std::string& path)
{
    ...
    std::list<size_t>& indexes, std::optional<std::string>& replaceStr)
{
    ...
    addRecordProbePath(record, device.path);
