
#include "em_config.hpp"

#include <nlohmann/json.hpp>
#include <phosphor-logging/lg2.hpp>

#include <expected>

// @returns error message on failure
template <typename T>
std::expected<void, std::string> getConfigProp(
    const nlohmann::json::object_t& config, const std::string& name,
    T& property)
{
    auto nameIt = config.find(name);

    if (nameIt == config.end())
    {
        return std::unexpected(std::format("property '{}' not found", name));
    }

    const T* ptr = nameIt->second.get_ptr<const T*>();

    if (ptr == nullptr)
    {
        return std::unexpected(
            std::format("property '{}' has wrong type", name));
    }

    property = *ptr;

    return std::expected<void, std::string>(std::in_place_t{});
}

std::optional<EMConfig> EMConfig::fromJson(const nlohmann::json& config)
{
    const nlohmann::json::object_t* obj =
        config.get_ptr<const nlohmann::json::object_t*>();

    if (obj == nullptr)
    {
        lg2::error("config was not an object");
        return std::nullopt;
    }

    try
    {
        return EMConfig(*obj);
    }
    catch (const std::exception& except)
    {
        lg2::error("{MSG}", "MSG", except);
    }

    return std::nullopt;
}

static bool parseProbe(std::vector<std::string>& probeStmt,
                       bool& isArrayProbeStmt,
                       const nlohmann::json::object_t& config)
{
    auto findProbe = config.find("Probe");

    if (findProbe == config.end())
    {
        lg2::error("No 'Probe' statement");
        return false;
    }

    if (findProbe->second.type() == nlohmann::json::value_t::array)
    {
        isArrayProbeStmt = true;
        const auto* probeArray =
            findProbe->second.get_ptr<const nlohmann::json::array_t*>();

        for (auto p : *probeArray)
        {
            const std::string* probeStrPtr = p.get_ptr<const std::string*>();

            if (probeStrPtr == nullptr)
            {
                lg2::error("'Probe' statement array member was not a string");
                return false;
            }

            probeStmt.push_back(*probeStrPtr);
        }
    }
    else
    {
        isArrayProbeStmt = false;
        const std::string* probeStrPtr =
            findProbe->second.get_ptr<const std::string*>();

        if (probeStrPtr == nullptr)
        {
            lg2::error("'Probe' statement was not a string");
            return false;
        }

        probeStmt.push_back(*probeStrPtr);
    }
    return true;
}

static bool parseExposesRecord(
    std::vector<nlohmann::json::object_t>& exposesRecords,
    const nlohmann::json::object_t* obj)
{
    std::string value;

    auto result = getConfigProp<std::string>(*obj, "Name", value);
    if (!result.has_value())
    {
        lg2::error("Error: {ERROR}", "ERROR", result.error());
        return false;
    }
    result = getConfigProp<std::string>(*obj, "Type", value);
    if (!result.has_value())
    {
        lg2::error("Error: {ERROR}", "ERROR", result.error());
        return false;
    }

    exposesRecords.emplace_back(*obj);

    return true;
}

static bool parseExposes(std::vector<nlohmann::json::object_t>& exposesRecords,
                         const nlohmann::json::object_t& config)
{
    auto findExposes = config.find("Exposes");

    if (findExposes == config.end())
    {
        lg2::error("No 'Exposes' array");
        return false;
    }

    const auto* exposesArray =
        findExposes->second.get_ptr<const nlohmann::json::array_t*>();

    if (exposesArray == nullptr)
    {
        lg2::error("'Exposes' array has wrong type");
        return false;
    }

    for (const nlohmann::json& item : *exposesArray)
    {
        const nlohmann::json::object_t* obj =
            item.get_ptr<const nlohmann::json::object_t*>();
        if (obj == nullptr)
        {
            lg2::error("record was not an object");
            return false;
        }
        if (!parseExposesRecord(exposesRecords, obj))
        {
            return false;
        }
    }
    return true;
}

static bool parseExtraInterfaces(
    std::flat_map<std::string, nlohmann::json::object_t>& extraInterfaces,
    const nlohmann::json::object_t& config)
{
    // parse extra interfaces
    for (const auto& [key, value] : config)
    {
        if (key.starts_with("xyz.openbmc_project."))
        {
            const nlohmann::json::object_t* obj =
                value.get_ptr<const nlohmann::json::object_t*>();
            if (obj == nullptr)
            {
                lg2::error("record was not an object");
                return false;
            }
            extraInterfaces.insert_or_assign(key, *obj);
        }
    }

    return true;
}

EMConfig::EMConfig(const nlohmann::json::object_t& config)
{
    std::expected<void, std::string> result;

    result = getConfigProp<std::string>(config, "Name", name);
    if (!result.has_value())
    {
        throw std::invalid_argument(result.error());
    }

    result = getConfigProp<std::string>(config, "Type", type);
    if (!result.has_value())
    {
        throw std::invalid_argument(result.error());
    }

    // Preserve the optional power-state hint used when pruning boards.
    auto powerStateIt = config.find("PowerState");
    if (powerStateIt != config.end())
    {
        const auto* state = powerStateIt->second.get_ptr<const std::string*>();
        if (state != nullptr)
        {
            powerState = *state;
        }
    }

    if (!parseProbe(probeStmt, isArrayProbeStmt, config))
    {
        throw std::invalid_argument("could not parse Probe stmt");
    }
    if (!parseExposes(exposesRecords, config))
    {
        throw std::invalid_argument("could not parse Exposes array");
    }

    if (!parseExtraInterfaces(extraInterfaces, config))
    {
        throw std::invalid_argument("could not parse extra DBus interfaces");
    }
}

nlohmann::json EMConfig::toJson() const
{
    return toJsonObject();
}

nlohmann::json::object_t EMConfig::toJsonObject() const
{
    nlohmann::json::object_t res;

    res["Name"] = name;
    res["Type"] = type;
    if (powerState)
    {
        res["PowerState"] = *powerState;
    }

    // probe statement should round-trip unchanged
    if (!isArrayProbeStmt && probeStmt.size() == 1)
    {
        res["Probe"] = probeStmt.at(0);
    }
    else
    {
        res["Probe"] = nlohmann::json::array_t();
        for (const auto& probe : probeStmt)
        {
            res["Probe"].push_back(probe);
        }
    }

    res["Exposes"] = nlohmann::json::array_t();
    for (const auto& record : exposesRecords)
    {
        res["Exposes"].push_back(record);
    }

    for (const auto& [k, v] : extraInterfaces)
    {
        res[k] = v;
    }

    return res;
}
