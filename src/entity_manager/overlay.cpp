                  ...
                  nlohmann::json& currConfiguration)
{
    ...
    {
        std::vector<DeviceConfig> muxConfigs = std::move(newMuxConfigs);
        for (auto& newMux : muxConfigs)
        {
            if (newMux.buildDevice())
            {
                std::vector<DeviceConfig> linkedConfigs =
...
bool DeviceConfig::buildDevice(const size_t retries)
{
    if (retries == 0U)
    {
        return false;
    ...
    if (!deviceIsCreated())
    {
        ...
        if (!createDevice())
        {
            ...
            createTimer->async_wait(
                ...
                return buildDevice(retries - 1);
            });
            return false;
        }
    }

    if (boost::ends_with(type, "MAX31790") && bus && address)
    {
        std::vector<std::filesystem::path> paths;
        if (!findFiles(std::filesystem::path("/sys/class/hwmon"),
                               R"(pwm\d+_enable)", paths, true))
        {
            std::cerr << "No fan hwmon in system\n";
        }

        // Enable PWM
        for (const auto& pwmPath : paths)
        {
            enablePwm(pwmPath);
        }
    }
...
std::vector<DeviceConfig>
    DeviceConfig::linkMux(nlohmann::json& systemConfiguration)
{
    std::error_code ec;
         ...
         channelIndex++)
    {
        ...
        if (ec && ec.message().compare("File exists"))
        {
        ...
        else
        {
            ...
            if (!ec.message().compare("File exists"))
            {
                ...
                if (busNumber != linkedBusNumber)
                {
                    ...
                    if (ec)
                    {
                        std::cerr << "Failure creating symlink for " << fp
                                  << " to " << link << "\n";
                        std::cerr << ec.message() << std::endl;
                        continue;
                    }
                 ...
                 entity != systemConfiguration.end(); entity++)
            {
                ...
                for (auto& configuration : *findExposes)
                {
                    ...
                    auto findChName =
                        findMuxChannel.value().find("ChannelName");
