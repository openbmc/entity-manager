...
EntityManager::EntityManager(
    ...
    systemMapper(*this, io, systemBus, objServer), io(io),
    powerStatus(*systemBus), propertiesChangedTimer(io)
{
    ...
    systemMapper.inventoryManager.tryIfaceInitialize(entityIface);

    initFilters(systemMapper.configuration.probeInterfaces);
}
...
void EntityManager::postToDbus(const nlohmann::json& newConfiguration)
{
    ...
    for (const auto& [assocPath, assocPropValue] :
         systemMapper.topology.getAssocs(newBoards))
    {
        ...
        auto ifacePtr = systemMapper.inventoryManager.createInterface(
            assocPath, "xyz.openbmc_project.Association.Definitions",
            findBoard->second);

        ifacePtr->register_property("Associations", assocPropValue);
        systemMapper.inventoryManager.tryIfaceInitialize(ifacePtr);
    }
}
    ...
    std::map<std::string, std::string>& newBoards)
{
    ...
    auto boardValues = systemMapper.systemConfiguration[boardId];
    ...
    std::shared_ptr<sdbusplus::asio::dbus_interface> inventoryIface =
        systemMapper.inventoryManager.createInterface(
    ...
    systemMapper.inventoryManager.createAddObjectMethod(
        jsonPointerPath, boardPath, boardNameOrig);

    systemMapper.inventoryManager.populateInterfaceFromJson(
        jsonPointerPath, boardIface, boardValues);
    jsonPointerPath += "/";
    ...
    for (const auto& [propName, propValue] : boardValues.items())
    {
        if (propValue.type() == nlohmann::json::value_t::object)
        {
            std::shared_ptr<sdbusplus::asio::dbus_interface> iface =
                systemMapper.inventoryManager.createInterface(
                    boardPath, propName, boardNameOrig);

            systemMapper.inventoryManager.populateInterfaceFromJson(
                jsonPointerPath + propName, iface, propValue);
        }
    }
    ...
    const std::string& boardType)
{
    ...
    if (itemType == "BMC")
    {
        ...
        systemMapper.inventoryManager.populateInterfaceFromJson(
            jsonPointerPath, bmcIface, item, getPermission(itemType));
    }
    else if (itemType == "System")
    {
        std::shared_ptr<sdbusplus::asio::dbus_interface> systemIface =
            systemMapper.inventoryManager.createInterface(
                ...
                boardNameOrig);
        systemMapper.inventoryManager.populateInterfaceFromJson(
            jsonPointerPath, systemIface, item, getPermission(itemType));
    }
    ...
    for (const auto& [name, config] : item.items())
    {
        ...
        if (config.type() == nlohmann::json::value_t::object)
        {
            ...
            systemMapper.inventoryManager.populateInterfaceFromJson(
                jsonPointerPath, objectIface, config, getPermission(name));
        }
    ...
    systemMapper.inventoryManager.populateInterfaceFromJson(
        jsonPointerPath, itemIface, item, getPermission(itemType));

    systemMapper.topology.addBoard(boardPath, boardType, boardNameOrig, item);
}
...
{
    if (systemConfiguration.empty() || systemMapper.lastJson.empty())
    {
    ...
    timer.async_wait(
        [&systemConfiguration, this](const boost::system::error_code& ec) {
            ...
            for (const auto& [name, device] : systemMapper.lastJson.items())
            {
                                       ...
                                       const nlohmann::json& device)
{
    ...
    auto& ifaces = systemMapper.inventoryManager.getDeviceInterfaces(device);
    ...
    ifaces.clear();
    systemMapper.systemConfiguration.erase(name);
    systemMapper.topology.remove(device["Name"].get<std::string>());
    logDeviceRemoved(device);
}
    ...
    const nlohmann::json newConfiguration)
{
    ...
    boost::asio::post(io, [this, &instance, count, &timer, newConfiguration]() {
        ...
        if (count == instance)
        {
            startRemovedTimer(timer, systemMapper.systemConfiguration);
        }
    });
}
...
void EntityManager::propertiesChangedCallback()
{
    ...
    propertiesChangedTimer.async_wait(
        [this, count](const boost::system::error_code& ec) {
            ...
            nlohmann::json oldConfiguration = systemMapper.systemConfiguration;
            auto missingConfigurations = std::make_shared<nlohmann::json>();
            *missingConfigurations = systemMapper.systemConfiguration;

            auto perfScan = std::make_shared<scan::PerformScan>(
                *this, *missingConfigurations,
                systemMapper.configuration.configurations, io,
    ...
    if (em_utils::fwVersionIsSame())
    {
        if (std::filesystem::is_regular_file(currentConfiguration))
        {
            ...
            if (jsonStream.good())
            {
                ...
                {
                    systemMapper.lastJson = std::move(data);
                }
            }
            else
            {
