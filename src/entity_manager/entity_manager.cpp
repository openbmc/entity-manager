 ...
 *
 * @return true  if the FRU service is ready and initialized (child nodes found).
...
bool EntityManager::waitForFruReadyAndInitialized(int maxRetries, int sleepMs)
{
    ...
    for (int i = 0; i < maxRetries; ++i)
    {
        try
        {
            ...
            {
                std::cerr << "FRU ready and initialized after " << i << " retries\n";
                fruReady = true;
                break;
            }
        }
    ...
    if (!fruReady)
    {
        std::cerr << "FRU not ready or not initialized after " << maxRetries << " retries\n";
    }
