...
void Topology::addConfiguredPort(const Path& path,
                                 const nlohmann::json& exposesItem)
{
    const auto findConnectsToName = exposesItem.find("ConnectsToName");
    if (findConnectsToName == exposesItem.end())
    {
        lg2::error("Board at path {PATH} is missing ConnectsToName", "PATH",
                   path);
        return;
    }
    const std::vector<std::string> connectsToName =
        findConnectsToName->get<std::vector<std::string>>();
    ...
    for (const auto& name : connectsToName)
    {
        addPort(name, path, assoc.value());
    }
}
