  ...
  private:
    ...
    void fillAssocsForPortId(
        std::unordered_map<sdbusplus::object_path, std::set<Association>>&
            result,
        BoardPathsView boardPaths,
        const std::map<sdbusplus::object_path, std::set<AssocName>>&
            pathAssocs);
    ...
    void addConfiguredPort(const sdbusplus::object_path& path,
                           const nlohmann::json& exposesItem);
    void addPort(const PortType& port, const sdbusplus::object_path& path,
    ...
    std::unordered_map<PortType,
                       std::map<sdbusplus::object_path, std::set<AssocName>>>
        ports;
    ...
    // it was probed on.
    std::unordered_map<sdbusplus::object_path, std::set<sdbusplus::object_path>>
        probePaths;
};
