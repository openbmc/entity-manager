...
using BoardPathsView = decltype(std::views::keys(
    std::declval<std::map<sdbusplus::object_path, std::string>&>()));
...
class Topology
{
  ...
  private:
    ...
    void addDownstreamPort(const object_path& path,
                           const nlohmann::json& exposesItem);

    // @brief: fill associations map with the associations for a port identifier
    // such as 'MB Upstream Port'
    void fillAssocsForPortId(
        std::unordered_map<object_path, std::set<Association>>& result,
        BoardPathsView boardPaths,
        const std::map<object_path, std::set<AssocName>>& pathAssocs);

    void fillAssocForPortId(
        std::unordered_map<object_path, std::set<Association>>& result,
        BoardPathsView boardPaths, const object_path& upstream,
        const object_path& downstream, const AssocName& assocName);

    void addConfiguredPort(const object_path& path,
                           const nlohmann::json& exposesItem);
    void addPort(const PortType& port, const object_path& path,
    ...
    std::unordered_map<PortType, std::map<object_path, std::set<AssocName>>>
        ports;

    std::unordered_map<object_path, BoardType> boardTypes;
    std::unordered_map<BoardName, object_path> boardNames;

    // Represents the mapping between inventory object paths of a
    // probed configuration and the object paths of DBus interfaces
    // it was probed on.
    std::unordered_map<object_path, std::set<object_path>> probePaths;
};
