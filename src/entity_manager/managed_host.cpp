...
const sdbusplus::object_path powerBasePath =
    sdbusplus::object_path(HostState::namespace_path::value) /
    HostState::namespace_path::host;

sdbusplus::object_path hostPowerPath(size_t hostIndex)
{
    return std::format("{}{}", powerBasePath.string(), hostIndex);
}

ssize_t hostIndexFromPath(const sdbusplus::object_path& path)
{
    if (!path.string().starts_with(powerBasePath.string()))
    {
        return -1;
    }
    std::string hostNum = path.string().substr(powerBasePath.string().size());
    try
    {
