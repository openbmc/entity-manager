...
void EMDBusInterface::populateIntfPDICompatObject(
    ...
    sdbusplus::asio::object_server& objServer, const std::string& boardNameOrig,
    sdbusplus::asio::PropertyPermission permission, size_t depth)
{
    if (!value.contains("Type"))
    {
        debug("Did not find 'Type' field on path {PATH}, interface {IFACE}",
    ...
    const std::string ifaceNameAlt = std::format(
        "xyz.openbmc_project.Configuration.{}", value.value("Type", ""));
    std::shared_ptr<sdbusplus::asio::dbus_interface> objectIfaceAlt =
        createInterface(objServer, ifacePathAlt, ifaceNameAlt, boardNameOrig);
    populateIntfPDICompat(io, systemConfiguration, jsonPointerPath,
                          objectIfaceAlt, value, objServer, boardNameOrig,
                          permission, depth + 1);
}
    ...
    sdbusplus::asio::PropertyPermission permission, size_t depth)
{
    for (const auto& [index, v] : value.items())
    {
        ...
        const std::string ifaceNameAlt = std::format(
            "xyz.openbmc_project.Configuration.{}", v.value("Type", ""));
        std::shared_ptr<sdbusplus::asio::dbus_interface> objectIfaceAlt =
            createInterface(objServer, ifacePathAlt, ifaceNameAlt,
                            boardNameOrig);
        populateIntfPDICompat(io, systemConfiguration, jsonPointerPath,
                              objectIfaceAlt, v, objServer, boardNameOrig,
                              permission, depth + 1);
    }
}
    ...
    for (const auto& [key, value] : dict.items())
    {
        ...
        if (type == nlohmann::json::value_t::object)
        {
            populateIntfPDICompatObject(
                io, systemConfiguration, jsonPointerPath, iface, key, value,
                objServer, boardNameOrig, permission, depth);
            continue;
        }
        ...
        {
            populateIntfPDICompatArray(io, systemConfiguration, jsonPointerPath,
                                       iface, key, value, objServer,
                                       boardNameOrig, permission, depth);
            continue;
        }
