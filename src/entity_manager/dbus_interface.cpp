    ...
    sdbusplus::asio::PropertyPermission permission, size_t depth)
{
    ...
    populateIntfPDICompat(systemConfiguration, jsonPointerPath, objectIfaceAlt,
                          value, boardNameOrig, permission, depth + 1);
}
    ...
    sdbusplus::asio::PropertyPermission permission, size_t depth)
{
    for (const auto& [index, v] : value.items())
    {
        ...
        populateIntfPDICompat(systemConfiguration, jsonPointerPath,
                              objectIfaceAlt, v, boardNameOrig, permission,
                              depth + 1);
    }
}
    ...
    sdbusplus::asio::PropertyPermission permission, size_t depth)
{
    ...
    for (const auto& [key, value] : dict.items())
    {
        ...
        {
            populateIntfPDICompatObject(systemConfiguration, jsonPointerPath,
                                        iface, key, value, boardNameOrig,
                                        permission, depth);
            continue;
        }
        ...
        {
            populateIntfPDICompatArray(systemConfiguration, jsonPointerPath,
                                       iface, key, value, boardNameOrig,
                                       permission, depth);
            continue;
        }
