    ...
    sdbusplus::asio::PropertyPermission permission)
{
    ...
    switch (modifiedType)
    {
        ...
        {
            addArrayOrPropertyToDbus<int64_t>(key, value, iface, permission,
                                              systemConfiguration, path);
            break;
        }
        case (nlohmann::json::value_t::number_unsigned):
        {
            addArrayOrPropertyToDbus<uint64_t>(key, value, iface, permission,
                                               systemConfiguration, path);
            break;
        }
        case (nlohmann::json::value_t::number_float):
        {
            addArrayOrPropertyToDbus<double>(key, value, iface, permission,
                                             systemConfiguration, path);
            break;
        }
        case (nlohmann::json::value_t::string):
        {
            addArrayOrPropertyToDbus<std::string>(key, value, iface, permission,
                                                  systemConfiguration, path);
            break;
        }
        default:
