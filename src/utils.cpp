...
std::pair<size_t, size_t> ifindFirst(std::string_view str, std::string_view sub)
{
    const auto* it = std::search(
        str.begin(), str.end(), sub.begin(), sub.end(), [](char a, char b) {
                 ...
                 std::string_view replace)
{
    ...
    while (pos < str.size())
    {
        auto [start, end] = ifindFirst(str.substr(pos), search);
        ...
        start += pos;
        str.replace(start, search.size(), replace);
        pos = start + replace.size();
    }
}
