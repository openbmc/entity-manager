...
arithmetic operator. It also supports precedence of operator like 2 * 3 + 1 = 7.
...
1. PSU$ADDRESS % 4 + 1 Temperature (partial string replacement)
2. PSU $ADDRESS % 4 + 1 Temperature
3. ($ADDRESS % 4 -1) / 2 (Expression with brackets)
4. Name: $PRODUCT_NAME Bus $bus Slot $Slot (Multiple string replacement)

It uses BOOST::SPIRIT::X3 parser which allows to add new rules without having
