...
intended to be simple, and are guided by the following principals.
...
2. Configuration files should trend toward one config file per physical piece of
   ...
   given piece of hardware in a single file, even at the risk of duplicating information.  Why?
    ...
    - Note: This is a "guideline" not a "rule".  There are many cases of
      hardware that despite having different part numbers, if functionally
      identical, and as such, the config files will never differ.

3. Configuration files are not a long-term stable ABI.  Why?
    - OpenBMC already has several relatively stable ABIs, including DBus.
...
4. Configurations should represent only the things that are _different_ and
   undetectable between platforms.  Why?
    - There are many behaviors that the BMC has that are very easily detected
      at runtime, or where the beahvior can be identical between different
      platforms.  In general, reducing the config files to _only_ the
      differences reduces complexity, and explicitly bounds where dicsussion is
      needed for platform differences, and where a difference is "supported"
      and "reasonable" to maintain in the long run.

## Configuration Records - Schema
All entities require the fields name, probe and exposes.

- **Name** is a user-facing name of the given entity.  As a matter of design, no
  reactor shall use the name in any way that changes the control flow.  This is
  to match with the Redfish definition of an abstract path name, and to avoid
  placing hidden constraints on the name within code.
- **Type** is a field to allow classifying a specific "type" to the hardware,
  mapping into the dbus interface.  (note, as a general rule, this obfuscates
  intent, and tends to be misused.  It may be removed in the future.)

- **Probe** is a field that describes the DBus matching required to identify
  this specific component.

- **Exposes** is a list of software resources that this config file should
  instantiate when a device is found.  Each exposes record requires:
    - **Name** is a user-facing name of the given exposes.  As a matter of
      design, no reactor shall use the name in any way that changes the control
      flow.  This is to match with the Redfish definition of an abstract path
      name, and to avoid placing hidden constraints on the name within code.
    - **Type** is the primary mechanism for reactors to identify and
      differentiate between different exposes records.
...
devices attached to the chassis). When one of the hardware modules needs to be
upgraded (e.g., a new temperature sensor), only such JSON configuration file
needs to be be updated.
