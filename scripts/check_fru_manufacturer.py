...
MANUFACTURER_KEYS = {"BOARD_MANUFACTURER", "PRODUCT_MANUFACTURER"}
...
def probe_has_manufacturer(probe_str):
    return any(key in probe_str for key in MANUFACTURER_KEYS)
...
def check_probes(config_file, config, configs_dir):
    ...
    for entry in get_entries(config):
        ...
        for probe_str in probe_strings:
            ...
            if not probe_has_manufacturer(probe_str):
                failures.append(
                    (
                        config_file,
            ...
            for match in MANUFACTURER_RE.finditer(probe_str):
...
def main():
    ...
    if not config_files:
        print(f"No configuration files found in {configs_dir}", file=sys.stderr)
        sys.exit(2)
