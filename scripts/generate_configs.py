...
def generateSingleConfig(config, dir_path):
    ...
    newconfig["Exposes"] = sorted(appliedRecords, key=lambda k: k["Type"])
...
def generateFromTemplate(filename):
    ...
    with open(filename) as fd:
        ...
        if isinstance(config, dict):
            if "Exposes" not in config:
                ...
                sys.exit(1)
            configs.append(
                generateSingleConfig(config, os.path.dirname(filename))
            )
        ...
        if isinstance(config, list):
            for c in config:
                if "Exposes" not in c:
                    ...
                    sys.exit(1)
                configs.append(
                    generateSingleConfig(c, os.path.dirname(filename))
                )
    ...
    with open(gen_filename, "w") as outfd:
        outfd.write(
            json.dumps(res, indent=4, sort_keys=True, separators=(",", ": "))
        )
        outfd.write("\n")
